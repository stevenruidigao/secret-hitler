...
OldAccount.find()
	.cursor()
	.eachAsync((account, i) => {
		if (account.version && account.version >= 3) {
			return;
		}
		...
		newAccount.overall = removeUnused({
			...
			isRainbow: newAccount.isRainbowOverall,
			dateRainbow: newAccount.dateRainbowOverall,
			...newAccount.overall,
		});
