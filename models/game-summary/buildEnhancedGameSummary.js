...
function buildEnhancedGameSummary(_summary) {
  ...
  const summary = fromJS(_summary, (key, value, path) => {
    const options = [
      'presidentHand', 'chancellorHand', 'enactedPolicy', 'presidentClaim',
      'chancellorClaim', 'presidentVeto', 'chancellorVeto', 'policyPeek',
      'policyPeekClaim', 'investigatorId', 'investigationId',
      'investigationClaim', 'specialElection', 'execution'
    ];
  ...
  const players = (() => {
    const roleToLoyalty =
        Map({liberal : 'liberal', fascist : 'fascist', hitler : 'fascist'});

    return summary.players.map((p, i) => {
      return Object.assign(
          {}, p,
          {id : i, loyalty : roleToLoyalty.get(p.role), icon : p.icon || 0});
    });
  })();
  ...
  // Boolean
  const isRebalanced =
      summary.gameSetting.rebalance6p || summary.gameSetting.rebalance7p ||
      summary.gameSetting.rebalance9p || summary.gameSetting.rerebalance9p;
  ...
  // Option[Int]
  const playerOf = id => {
    return fromNullable(Number.isInteger(id)
                            ? players.get(id)
                            : players.find(p => p.username === id));
  };

  // Option[String]
  const usernameOf = id => { return playerOf(id).map(p => p.username); };

  // Option[String]
  const tagOf =
      id => { return playerOf(id).map(p => `${p.username} [${p.id}]`); };

  // Option[String]
  const loyaltyOf = id => { return playerOf(id).map(p => p.loyalty); };

  // Option[String]
  const roleOf = id => { return playerOf(id).map(p => p.role); };

  // Option[List[Option[{ ja: Boolean, presidentId: Int, chancellorId: Int }]]]
  const votesOf = username => {
    return indexOf(username).map(
        i => turns.filter(t => t.votes.get(i)).map(t => {
  ...
  const shotsOf = username => {
    return indexOf(username).map(
        i => turns.filter(t => t.presidentId === i && t.execution.isSome())
                 .map(t => t.execution.value()));
  };

  // Option[Boolean]
  const isWinner =
      username => { return loyaltyOf(username).map(l => l === winningTeam); };
