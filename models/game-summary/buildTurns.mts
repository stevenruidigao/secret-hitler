...
const initialDeckSize = (gameSetting: any) => {
...
const initialTrack = (gameSetting: any) => {
...
const buildTurn = (prevTurnOpt: any, log: any, players: any, gameSetting: any) => {
	...
	const { beforePlayers, afterPlayers } = (() => {
		const p = (deadPlayers: any[]) => players.map((p: any, i: number) => Object.assign({}, p, { isDead: deadPlayers.includes(i) }));
	...
	// List[Option[Boolean]]
	const votes = log.votes.map((v: any, i: number) => (beforeDeadPlayers.includes(i) ? none : some(v)));

	// Int
	const jas = flattenListOpts(votes).count((v: any) => v);
	...
	const { beforeTrack, afterTrack } = (() => {
		const f = (count: number, policy: string, type: string) => {
			const inc = filterOpt(policy, (x: any) => x === type)
				.map((x: any) => 1)
				.valueOrElse(0);
	...
	const isHitlerElected = (() => {
		const hitlerIndex = players.findIndex((p: any) => p.role === 'hitler');
	...
	const isHitlerKilled = (() => {
		...
		return log.execution.map((e: any) => e === hitlerIndex).valueOrElse(false);
	})();
	...
	const isMerlinShot =
		...
		(() => {
			const merlinIndex = players.findIndex((p: any) => p.role === 'merlin');

			return log.assassination.map((e: any) => e === merlinIndex).valueOrElse(false);
		})();
