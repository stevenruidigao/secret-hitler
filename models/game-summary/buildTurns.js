...
const buildTurns = (turns, logs, players, gameSetting) => {
  ...
  const nextTurn =
      buildTurn(fromNullable(turns.last()), logs.first(), players, gameSetting);
...
const buildTurn = (prevTurnOpt, log, players, gameSetting) => {
  ...
  const {beforePlayers, afterPlayers} = (() => {
    const p = deadPlayers => players.map(
        (p, i) => Object.assign({}, p, {isDead : deadPlayers.includes(i)}));
  ...
  // List[Option[Boolean]]
  const votes =
      log.votes.map((v, i) => (beforeDeadPlayers.includes(i) ? none : some(v)));
  ...
  const poorMansVeto =
      isVotePassed &&
      log.enactedPolicy
          .isNone(); // backwards compatability before veto was tracked
  ...
  const isHitlerElected = (() => {
    ...
    return beforeTrack.reds >= 3 && log.chancellorId === hitlerIndex &&
           isVotePassed;
  })();
  ...
  const {presidentDiscard, chancellorDiscard} = (() => {
    ...
    const presidentDiscard =
        handToPolicyOpt(handDiffOpt(log.presidentHand, log.chancellorHand));
