...
module.exports.TOU_CHANGES = [
  ...
  },
  {
    changeVer : '1.2',
    changeDesc :
        'Terms of Use now states that explicitly forbidden words may result in action without reports.'
  },
  {
    changeVer : '1.1',
    changeDesc :
...
module.exports.CURRENTSEASONNUMBER = 14;
...
module.exports.PLAYERCOLORS = (user, isSeasonal, defaultClass, eloDisabled) => {
  if (Boolean(user.staffRole && user.staffRole.length &&
              user.staffRole !== 'trialmod' && user.staffRole !== 'altmod') &&
      !user.staffDisableStaffColor) {
    return cn(defaultClass, {
      ...
      anji : user.userName === 'Anji' && user.staffRole === 'editor'
             ...
             (!(user.staffRole && user.staffRole.length &&
                user.staffRole !== 'trialmod' && user.staffRole !== 'altmod') ||
              !user.staffDisableStaffColor)) {
    ...
    return w + l >= 50 ? eloDisabled ? cn(defaultClass, {
      experienced1 : w + l > 49,
      experienced2 : w + l > 99,
...
module.exports.getBadWord = text => {
  const badWords = {
    ...
    nigger : [
      'nigga', 'nibba', 'nignog', 'n1bba', 'ni99a', 'n199a', 'nignug', 'bigga',
      'nigg', 'niggre', 'n1gger'
    ],
  ...
  const exceptions = [
    /(i|o)f (a|4) g/gi, /underclaim on gov/gi, /bastard/gi, /big ga/gi,
    /among/gi, /mongod/gi, /mongolia/gi, /off again/gi, /pokemon game/gi
  ]; // This list for all exceptions to bypass swear filter
  let foundWord = [
    null, null
  ]; // Future found bad word, in format of: [blacklisted word, variation]
  ...
  // This version only detects words if they are whole and have whitespace at
  // either end.
  /* Object.keys(badWords).forEach(key => {
          if (new RegExp(`(^|\\s)${key}s?(\\s|$)`,
  'i').test(text.toLowerCase())) { foundWord = [key, key];
          }
          else badWords[key].forEach(word => {
                  if (new RegExp(`(^|\\s)${word}s?(\\s|$)`,
  'i').test(text.toLowerCase())) { foundWord = [key, word];
                  }
          });
  });*/
  return foundWord;
