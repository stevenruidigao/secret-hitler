import {capitalize, handToText, mapOpt1, text} from '../../../utils';
...
export default function(snapshot, game, userInfo, hideHand) {
  ...
  const claimToText = claim =>
      (claim.valueOrElse([]).length !== 0
           ? claim.valueOrElse(text('player', 'nothing'))
           : text('player', 'nothing'));
  const claimHandToText = (claim, userInfo) =>
      claimToText(mapOpt1(claim => handToText(claim, userInfo))(claim));
  ...
  case 'election':
    if (snapshot.gameOver) {
      return gameOverText(
          [ text('hitler', 'Hitler'), text('normal', 'is elected.') ]);
    } else {
      return [
        text('normal', 'The vote'),
        text('normal', isVotePassed ? 'passes' : 'fails'), text('player', jas),
        text('normal', 'to'), text('player', neins)
      ];
    }
  case 'topDeck':
  ...
  case 'presidentLegislation':
    return [
      text('player', usernameOf(snapshot.presidentId)), text('normal', 'draws')
    ]
        .concat(hideHand ? [ text('player', 'three policies') ]
                         : handToText(snapshot.presidentHand, userInfo))
  ...
  case 'chancellorLegislation':
    return [
      ...
      text('normal', 'receives')
        ...
        .concat(hideHand ? [ text('player', 'two policies') ]
                         : handToText(snapshot.chancellorHand, userInfo))
        .concat([ text('normal', 'and claims') ])
  ...
  case 'investigation':
    ...
    if (investigator === snapshot.presidentId) {
      // normal investigation, and reverse investigations before the
      // introduction of "investigatorId"
      return [
      ...
      return [
        ...
        text('player', usernameOf(investigator)), text('normal', 'who claims'),
        claimToText(
            snapshot.investigationClaim.map(i => text(i, capitalize(i))))
      ];
    }
  case 'policyPeek':
    return [
      text('player', usernameOf(snapshot.presidentId)), text('normal', 'peeks')
    ]
        .concat(hideHand ? [ text('player', 'three policies') ]
                         : handToText(snapshot.policyPeek, userInfo))
        .concat([ text('normal', 'and claims') ])
  ...
  case 'execution':
    if (snapshot.gameOver) {
      return gameOverText(
          [ text('hitler', 'Hitler'), text('normal', 'is killed.') ]);
    } else {
      return [
        text('player', usernameOf(snapshot.presidentId)),
