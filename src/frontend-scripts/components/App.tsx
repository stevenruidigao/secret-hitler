...
import RightSidebar from './section-right/RightSidebar.tsx';
...
export class App extends React.Component<AppProps> {
	...
	constructor(props: AppProps) {
		...
		this.prevHash = '';
		this.lastReconnectAttempt = new Date().valueOf();
	}
	...
	componentDidMount() {
		...
		const { classList } = document.getElementById('game-container') || { classList: [] };
		...
		socket.on('userList', (list) => {
			...
			if (since > 1000 * 5) {
				this.lastReconnectAttempt = now.valueOf();
				const { userInfo } = this.props;
				if (userInfo && userInfo.userName) {
					if (!list.list.map((user: any) => user.userName).includes(userInfo.userName)) {
	...
	router() {
		 ...
		 */
		const updateStatus = (type: string, uid?: string) => {
		...
		if (hash.substr(0, 10) === '#/profile/') {
			dispatch(fetchProfile(hash.split('#/profile/')[1])); // TODO: uhhh `, userInfo.userName));`
	...
	handleSeatingUser(password?: string) {
		const { gameInfo } = this.props;
		const data = {
	...
	handleLeaveGame(manualLeaveGame?: any) {
