...
exports.filterOpt =
    (opt,
     predicate) => { return opt.flatMap(o => (predicate(o) ? opt : none)); };
...
exports.mapOpt1 = f => { return x => x.map(xx => f(xx)); };

// (x: A, y: B) => C => (x: Option[A], y: Option[B]) => Option[C]
exports.mapOpt2 =
    f => { return (x, y) => x.flatMap(xx => y.map(yy => f(xx, yy))); };
...
const handToPolicies = (exports.handToPolicies = hand => {
  if (hand.hasOwnProperty('reds') && hand.hasOwnProperty('blues')) {
    const toPolicies =
        (count, type) => { return Range(0, count).map(i => type).toList(); };
...
const isComma = (index, list, userInfo) => {
  const mode = (userInfo && userInfo.gameSettings &&
...
exports.handToText = (hand, userInfo) => {
  ...
  return handToPolicies(hand)
      .map((policy, index, list) =>
               text(policy, policyToString(policy, userInfo), false,
                    isComma(index, list, userInfo)))
      .concat(text('normal', ''))
      .toArray();
};
...
// (s: String) => String
exports.capitalize = s => { return s.charAt(0).toUpperCase() + s.slice(1); };
...
exports.objectContains = (target, subset) => {
  return Object.keys(subset).reduce(
      (acc, key) => acc && target[key] === subset[key], true);
};
