...
import mockGame from '../../mocks/mockGameSummary.mts';
...
export default () => {
	...
	describe('generic game', () => {
		...
		const testSnapshot = (snapshot: any, turnNum: any, phase: any, track: any, deadPlayers: any, electionTracker: any) => {
			expect(snapshot.turnNum).toBe(turnNum);
			expect(snapshot.phase).toBe(phase);
			expect(snapshot.track).toEqual(track);
			snapshot.players.forEach((p: any, i: number) => {
				if (deadPlayers.includes(i)) expect(p.isDead).toBe(true);
				else expect(p.isDead).toBe(false);
			});
		};

		const findPhase = (phase: any, turnNum: any) =>
			replay.find((t: any) => {
				return t.turnNum === turnNum && t.phase === phase;
			});

		const testCandidacy = (snapshot: any, turnNum: any, phase: any, track: any, deadPlayers: any, electionTracker: any, presidentId: any) => {
			testSnapshot(snapshot, turnNum, phase, track, deadPlayers, electionTracker);
			expect(snapshot.presidentId).toBe(presidentId);
		};

		const testNomination = (turnNum: any, presidentId: any, chancellorId: any) => {
			const snapshot = findPhase('nomination', turnNum);
			expect(snapshot.presidentId).toBe(presidentId);
			expect(snapshot.chancellorId).toBe(chancellorId);
		};

		const testElection = (turnNum: any, votes: any) => {
			const snapshot = findPhase('election', turnNum);
			(expect(snapshot.votes) as any).toListOptionEqual(votes);
		};

		const testLegislation = (govt: any, turnNum: any, hand: any, discard: any, claim: any) => {
			if (!hand) {
		...
		const testPolicyEnaction = (turnNum: any, policy: any) => {
			if (!policy) {
		...
		const testInvestigation = (turnNum: any, investigation: any, claim: any) => {
			if (!investigation) {
		...
		const testExecution = (turnNum: any, execution: any) => {
			if (!execution) {
		...
		describe('should take the correct snapshot for turn', () => {
			it('0', () => {
				...
				testInvestigation(0, null, undefined);
			});
			...
			it('3', () => {
				...
				testInvestigation(3, null, undefined);
			});
			...
			it('5', () => {
				...
				testPresidentLegislation(5, null, none, undefined);

				testChancellorLegislation(5, null, none, undefined);

				testPolicyEnaction(5, null);

				testExecution(5, null);

				testInvestigation(5, null, undefined);
			});
			...
			it('6', () => {
				...
				testInvestigation(6, null, undefined);
			});
			...
			it('7', () => {
				...
				testInvestigation(7, null, undefined);
			});
		});
