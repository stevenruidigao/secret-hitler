...
const Account =
    require('../models/account'); // eslint-disable-line no-unused-vars
...
module.exports = () => {
  ...
  app.get('/', (req, res) => { renderPage(req, res, 'page-home', 'home'); });

  app.post('/', (req, res) => { renderPage(req, res, 'page-home', 'home'); });

  app.get('/rules',
          (req, res) => { renderPage(req, res, 'page-rules', 'rules'); });

  app.get(
      '/changelog',
      (req, res) => { renderPage(req, res, 'page-changelog', 'changelog'); });

  app.get(
      '/how-to-play',
      (req, res) => { renderPage(req, res, 'page-howtoplay', 'howtoplay'); });

  app.get('/stats',
          (req, res) => { renderPage(req, res, 'page-stats', 'stats'); });

  app.get(
      '/stats-season',
      (req,
       res) => { renderPage(req, res, 'page-stats-season', 'stats-season'); });

  app.get('/about',
          (req, res) => { renderPage(req, res, 'page-about', 'about'); });

  app.get('/tou', (req, res) => { renderPage(req, res, 'page-tou', 'tou'); });

  app.get('/polls',
          (req, res) => { renderPage(req, res, 'page-polls', 'polls'); });

  app.get('/player-profiles', (req, res) => {
    renderPage(req, res, 'page-player-profiles', 'playerProfiles');
  });

  app.get('/game', ensureAuthenticated,
          (req, res) => { res.redirect('/game/'); });

  const getHSLcolors =
      hsl => [parseInt(hsl.split(',')[0].split('hsl(')[1], 10),
    ...
    if (req.user.isBanned) {
      ...
      Account.findOne({username}, (err, account) => {
        ...
        const backgroundColor =
            account.backgroundColor || DEFAULTTHEMECOLORS.baseBackgroundColor;
        ...
        const [backgroundHue, backgroundSaturation, backgroundLightness] =
            getHSLcolors(backgroundColor);
        const [textHue, textSaturation, textLightness] =
            getHSLcolors(textColor);
        ...
        const gameObj = {
          ...
          blacklist,
          primaryColor :
              account.primaryColor || DEFAULTTHEMECOLORS.primaryColor,
          secondaryColor :
              account.secondaryColor || DEFAULTTHEMECOLORS.secondaryColor,
          tertiaryColor :
              account.tertiaryColor || DEFAULTTHEMECOLORS.tertiaryColor,
          backgroundColor,
          secondaryBackgroundColor :
              `hsl(${backgroundHue}, ${backgroundSaturation}%, ${
                  backgroundLightness > 50 ? backgroundLightness - 7
                                           : backgroundLightness + 7}%)`,
          tertiaryBackgroundColor :
              `hsl(${backgroundHue}, ${backgroundSaturation}%, ${
                  backgroundLightness > 50 ? backgroundLightness - 14
        ...
        if ((account.ipHistory && account.ipHistory.length === 0) ||
            (account.ipHistory.length > 0 &&
             account.ipHistory[account.ipHistory.length - 1].ip !== ip)) {
          account.ipHistory.push({date : new Date(), ip : ip});
        }
        account.save(() => { res.render('game', gameObj); });
      });
    }
  });

  app.get('/observe', (req, res) => { res.redirect('/observe/'); });
  ...
  app.get('/observe/', (req, res) => {
    ...
    const secondaryBackgroundColor =
        `hsl(${backgroundHue}, ${backgroundSaturation}%, ${
            backgroundLightness > 50 ? backgroundLightness - 5
                                     : backgroundLightness + 5}%)`;
    const tertiaryBackgroundColor =
        `hsl(${backgroundHue}, ${backgroundSaturation}%, ${
            backgroundLightness > 50 ? backgroundLightness - 10
        ...
        Account.findOne({username}, (err, account) => {
          ...
          if (account) {
            ...
            Account.findOne({username : authedUser}).then(acc => {
              if (acc && acc.staffRole &&
                  (acc.staffRole === 'moderator' ||
                   acc.staffRole === 'editor' || acc.staffRole === 'admin' ||
                   acc.staffRole === 'trialmod')) {
                try {
                  _profile.lastConnectedIP =
                      '-' + obfIP(_profile.lastConnectedIP);
                } catch (e) {
  ...
  app.get('/modThread', (req, res) => {
    ...
    const mangle = chat => chat.replace(/&/g, '&amp;')
                               .replace(/</g, '&lt;')
                               .replace(/>/g, '&gt;')
                               .replace(/"/g, '&quot;')
                               .replace(/'/g, '&#39;');
    ...
    Account.findOne({username}).then(account => {
      if (account.staffRole === 'moderator' || account.staffRole === 'editor' ||
            ...
            .then(dm => {
              if (!dm) {
                ...
                for (const message of dm.messages) {
                  chatLog.push(`${message.userName}${
                      message.userName
                          ? (message.type === 'leave' || message.type === 'join'
                                 ? ' '
                                 : ': ')
                          : ''}${mangle(message.chat)}`);
                }
        ...
        res.status(401).send(
            'You cannot access this resource. Ensure you are logged in.');
      }
    });
  });
  ...
  app.get('/viewPatchNotes', ensureAuthenticated, (req, res) => {
    Account.updateOne({username : req.user.username},
                      ...
                      err => { res.sendStatus(err ? 404 : 202); });
  });
    ...
    try {
      ...
      Account.findOne({username})
          .then(account => {
            if (account.wins + account.losses < 50) {
            ...
            } else if (new Date(account.gameSettings.customCardbackSaveTime) &&
                       Date.now() -
                               new Date(
              ...
              res.json({
                message :
                    'You can only change your cardback once every 30 seconds.'
              });
            } else {
              ProcessImage(
                  username, raw,
                  (resp, err) => { res.json({message : err || resp}); });
            }
          })
          .catch(err => { console.log(err, 'account err in cardbacks'); });
    } catch (error) {
