...
import { expandAndSimplify, obfIP, doesIPMatchCIDR } from './socket/ip-obf.ts';
import { accountCreationDisabled, bypassVPNCheck, verifyBypass, consumeBypass, testIP } from './socket/models.ts';
import { handleDefaultIPv6Range } from './socket/util.ts';
...
const checkIP = (config: any) => {
	...
	if (hasBypass) {
		...
		creationDisabledSignup.save().then(() => {
			res.status(403).json({
				message:
		...
		BannedIP.find({
			...
			if (bans.some(checkFragban) && !hasBypass) {
				...
				testIP(signupIP, (banType: string | null, unbanTime: number, permanent: boolean) => {
...
const continueSignup = (config: any) => {
	...
	if (vpnScore >= 0.995 && !hasBypass) {
		...
		if (isOAuth) {
			...
			Account.register(new Account(accountObj), Math.random().toString(36).substring(2))
				.then((account) => {
					...
					passport.authenticate(type)(req, res, () => {
						oauthSignup.save().then(() => {
							newPlayerBan.save().then(() => {
								req.login(account, () => {
									res.redirect('/game');
								});
							});
			...
			Account.register(new Account(save), password)
				.then(() => {
					if (hasBypass) consumeBypass(bypassKey, username, signupIP);
...
export const accounts = (torIpsParam: any) => {
	verifyRoutes();
	torIps = torIpsParam;

	app.get('/account', ensureAuthenticated, (req: any, res) => {
	...
	app.post('/account/change-password', ensureAuthenticated, (req: any, res, next) => {
	...
	app.post('/account/reset-password', (req, res, next) => {
		...
		Account.findOne({
			...
			.catch((err: Error) => console.log(err, 'account err'));
	});

	app.post('/account/signup', (req: any, res, next) => {
	...
	app.post(
		'/account/signin',
		(req: any, res, next) => {
			testIP(req.expandedIP, (banType: string, unbanTime: number, permanent: boolean) => {
	...
	app.post('/account/add-email', ensureAuthenticated, (req: any, res, next) => {
		...
		if (email.split('@')[1] && bannedEmails.includes(email.split('@')[1]) && process.env.NODE_ENV === 'production') {
			...
			Account.findOne({ 'verification.email': email })
				.then((account) => {
	...
	app.post('/account/change-email', ensureAuthenticated, (req: any, res, next) => {
		const { email } = req.body;
		const { username } = req.user;
	...
	app.post('/account/request-verification', ensureAuthenticated, (req: any, res, next) => {
	...
	app.post('/account/logout', ensureAuthenticated, (req: any, res) => {
		req.logOut();
		res.send();
	});
	...
	app.get('/getipintel-status', (req, res) => {
		res.send(
			`Current GetIPIntel counter is at ${getIPIntelCounter.count} - resets in ${(getIPIntelCounter.reset.valueOf() - new Date().valueOf()) / 1000 / 60} minutes`,
		);
	});
	...
	const oAuthAuthentication = (req: any, res: Response, next: any, type: string) => {
		const ip = req.expandedIP;
		testIP(ip, (banType: string, unbanTime: number) => {
			if (banType && banType !== 'new' && banType !== 'fragbanSmall' && banType !== 'fragbanLarge') {
				if (banType === 'nocache') res.status(403).json({ message: 'The server is still getting its bearings, try again in a few moments.' });
				else if (banType === 'small' || banType === 'big') {
					res
										...
										Account.findOne({ username: new RegExp(profile.username, 'i') })
											.then((account) => {
												...
												if (account) {
													...
													const new88 = new EightEightCounter({
														date: new Date(),
														username: profile.username, // TODO: check, used to be `username`
													});
													new88.save().then(() => {
														req.session.oauthProfile = profile;
														res.redirect('/oauth-select-username');
	...
	app.get('/oauth-select-username', (req: any, res, next) => {
	...
	app.post('/oauth-select-username', (req: any, res, next) => {
	...
	app.get('/revoke-discord', ensureAuthenticated, (req: any, res) => {
		req.user.discord = {};
		req.user.save(() => {
	...
	app.get('/revoke-github', ensureAuthenticated, (req: any, res) => {
		req.user.github = {};
		req.user.save(() => {
			res.redirect('/account');
		});
