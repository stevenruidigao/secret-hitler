...
let getIPIntelCounter = {
  reset : new Date(new Date().getFullYear(), new Date().getMonth(),
                   new Date().getDate(), 23, 59, 59, 999),
  count : 0
};
...
const checkIP = config => {
  ...
  if (hasBypass) {
    ...
    const torSignup = new Signups({
      ...
      type : 'Failed - TOR',
      ip : obfIP(signupIP),
      email : config.isOAuth ? `${config.type} ${config.profile.username}${
                                   config.type === 'discord'
                                       ? '#' + config.profile.discriminator
                                       : ''}`
                             : Boolean(email),
      unobfuscatedIP : signupIP,
      oauthID : `${
          config.isOAuth && config.type === 'discord' ? config.profile.id : ''}`
    });
    torSignup.save(() => {
      res.status(403).json(
          {message : 'Use of TOR is not allowed on this site.'});
             ...
             !process.env.IPBANSINDEV) {
    config.vpnScore = 0;
    next(config);
  } else if (getIPIntelCounter.count >= 1995 &&
             new Date() < getIPIntelCounter.reset && !VPNCache[signupIP]) {
    ...
    const checkFragban = ban => {
      return (((new Date() < ban.bannedDate || ban.permanent) &&
               doesIPMatchCIDR(ban.ip, signupIP)) ||
              ((new Date() < ban.bannedDate || ban.permanent) &&
               !ban.ip.includes('/') && ban.ip.includes('.') &&
               signupIP.includes('.') && // backwards compatability
               (ban.ip === signupIP.split('.').slice(0, 3).join('.') ||
                ban.ip === signupIP.split('.').slice(0, 2).join('.'))));
    };
    BannedIP.find({type : [ 'fragbanSmall', 'fragbanLarge' ]}).then(bans => {
      if (bans.some(checkFragban) && !hasBypass) {
        ...
        testIP(signupIP, (banType, unbanTime, permanent) => {
          if (hasBypass && banType == 'new')
            banType = null;
          if (banType && !hasBypass) {
            if (banType == 'nocache')
              res.status(403).json({
                message :
                    'The server is still getting its bearings, try again in a few moments.'
              });
            else if (banType === 'small' || banType === 'big') {
              res.status(403).json({
                message :
                    'You can no longer access this service.  If you believe this is in error, contact the moderators on our discord channel.'
              });
              ipBanned = true;
            } else if (banType === 'tiny') {
              res.status(403).json({
                message :
                    `Your IP address was timed out.  If you believe this is in error, contact the moderators on Discord. Your timeout expires on ${
                        new Date(unbanTime)}`
              });
              ipBanned = true;
            } else if (banType == 'new') {
          ...
          if (ipBanned) {
            const ipbannedSignup = new Signups({
              date : new Date(),
              userName : username,
              type : `Failed - IPBanned ${permanent ? 'permanent ' : ''}${
                  banType}`,
              ip : obfIP(signupIP),
              email : config.isOAuth
                          ? `${config.type} ${config.profile.username}${
                                config.type === 'discord'
                                    ? '#' + config.profile.discriminator
                                    : ''}`
                          : Boolean(email),
              unobfuscatedIP : signupIP,
              oauthID : `${
                  config.isOAuth && config.type === 'discord'
                      ? config.profile.id
                      : ''}`
            });
            ipbannedSignup.save();
            ...
            if (bypassVPNCheck.status) {
              ...
              // (this is only reachable if we are not in production or we dont
              // have IPBANSINDEV set and if we dont have IPBANSINDEV set we are
              // guaranteed to be in production)
              fetch(`https://check.getipintel.net/check.php?ip=${
                        signupIP}&contact=${
                        process.env.GETIPINTELAPIEMAIL}&flags=f&format=json`)
                  .then(res => res.json())
                  .then(json => {
                    if (new Date() < getIPIntelCounter.reset) {
                      ...
                      getIPIntelCounter = {
                        reset : new Date(new Date().getFullYear(),
                                         new Date().getMonth(),
                                         new Date().getDate(), 23, 59, 59, 999),
                        count : 1
                      };
                    ...
                    if (vpnScore < 0) {
                      res.status(501).json({
                        message :
                            'There was an error processing your request. Please contact our moderators on Discord.'
                      });
                      console.log(
                          'Error in Get IP Intel, score given: ', vpnScore,
                          'IP: ', signupIP, 'Message: ', json.message);
                      return;
                    }
                  ...
                  .catch(e => {
                    ...
                    res.status(501).json({
                      message :
                          'There was a fatal error in processing your request. Please contact our moderators on Discord'
                    });
                    return;
                  });
            } else {
...
const continueSignup = config => {
  ...
  if (vpnScore >= 0.995 && !hasBypass) {
    ...
    vpnSignup.save(() => {
      res.status(403).json({
        message :
            'Use of a VPN is currently not allowed on this site. Contact the moderators on Discord for an exception.'
      });
    });
  } else {
    if (isOAuth) {
      ...
      const oauthSignup = new Signups({
        ...
        unobfuscatedIP : signupIP,
        oauthID : `${
            config.isOAuth && config.type === 'discord' ? accountObj.discordUID
                                                        : ''}`
      });

      Account.register(
          new Account(accountObj), Math.random().toString(36).substring(2),
          (err, account) => {
            if (err) {
              ...
              if (hasBypass)
                consumeBypass(bypassKey, username, signupIP);
...
module.exports.accounts = torIpsParam => {
  ...
  app.post('/account/change-password', ensureAuthenticated,
           (req, res, next) => {
  ...
  app.post('/account/delete-account', passport.authenticate('local'),
           (req, res) => {
             Account.findOne({username : req.user.username}).then(acc => {
               if (acc.isBanned ||
                   (acc.isTimeout && Date.now() < new Date(acc.isTimeout))) {
                 res.status(403).json(
                     {message : 'You cannot delete a banned account. '});
               } else {
                 Account.deleteOne({username : req.user.username}).then(() => {
                   Profile.deleteOne({_id : req.user.username})
                       .then(() => { res.send(); });
                 });
               }
             });
    ...
    Account.findOne({'verification.email' : req.body.email})
        .then(account => {
          if (!account) {
            res.status(404).json({
              message :
                  'There is no verified account associated with that email.'
            });
          } else {
            setVerify({
  ...
  app.post('/account/signup', (req, res, next) => {
    ...
    if (bypassKey) {
      ...
      if (bypassKey.length) {
        if (!verifyBypass(bypassKey)) {
          res.status(401).json({
            message :
                'Restriction bypass key invalid, leave that field empty if it is not needed.'
          });
          return;
        }
        hasBypass = true;
    ...
    if (!/^[a-z0-9]+$/i.test(username)) {
      res.status(401).json(
          {message : 'Your username can only be alphanumeric.'});
    ...
    } else if (email && email.split('@')[1] &&
               bannedEmails.includes(email.split('@')[1]) &&
               process.env.NODE_ENV === 'production') {
      res.status(401).json({
        message :
            'Only non-disposable email providers are allowed to create verified accounts.'
      });
    } else if (blacklistedWords.some(
                   word => new RegExp(word, 'i').test(username))) {
      res.status(401).json({
        message :
            'Your username contains a naughty word or part of a naughty word.'
      });
      ...
      Account.find(queryObj, (err, accounts) => {
        ...
        if (accounts.length) {
          if (accounts.some(acc => acc.username.toLowerCase() ===
            ...
            res.status(401).json({
              message :
                  'That email address is being used by another verified account, please change that or use another email.'
            });
            return;
          }
        }
        ...
        testIP(req.expandedIP, (banType, unbanTime, permanent) => {
          if (banType && banType != 'new') {
            if (banType == 'nocache')
              ...
              res.status(403).json({
                message :
                    'You can no longer access this service.  If you believe this is in error, contact the moderators on Discord.'
              });
            }
          } else
            return next();
        ...
        Account.findOne({username : req.user.username}).then(player => {
          if (req.ipBanned && req.ipBanned !== '') {
            const ipbannedLogin = new Signups({
              date : new Date(),
              userName : req.user.username,
              type : `Failed Login - IPBanned ${
                  req.permanentIPBan ? 'permanent ' : ''}${req.ipBanned}`,
            ...
            if ((req.ipBanned === 'small' || req.ipBanned === 'big') &&
                !player.gameSettings.ignoreIPBans) {
              req.logOut();
              res.status(403).json({
                message :
                    'You can no longer access this service.  If you believe this is in error, contact the moderators on Discord.'
          ...
          if (player.isBanned) {
            ...
            res.status(403).json({
              message :
                  'Your account has been banned.  If you believe this is in error, contact the moderators on Discord.'
          ...
          if ((player.ipHistory && player.ipHistory.length === 0) ||
              (player.ipHistory.length > 0 &&
               player.ipHistory[player.ipHistory.length - 1].ip !== ip)) {
            player.ipHistory.push({date : new Date(), ip : ip});
          }
          player.save(() => { res.send(); });
          ...
          Profile.findOne({_id : req.user.username})
              ...
              .catch(err => { console.log(err, 'profile find err'); });

          if (player.isTimeout && new Date() < player.isTimeout) {
            req.logOut();
            res.status(403).json({
              message :
                  `Your account has been timed out.  If you believe this is in error, contact the moderators on Discord. Your timeout expires on ${
                      new Date(player.isTimeout)}`
              // TODO: include the reason moderators provided for the account
  ...
  app.post('/account/add-email', ensureAuthenticated, (req, res, next) => {
    ...
    if (email.split('@')[1] && bannedEmails.includes(email.split('@')[1]) &&
        process.env.NODE_ENV === 'production') {
      res.status(401).json({
        message :
            'Only non-disposible email providers are allowed to create verified accounts.'
      });
      ...
      Account.findOne({'verification.email' : email}, (err, account) => {
        ...
        if (account && process.env.NODE_ENV === 'production') {
          ...
          Account.findOne({username})
              ...
              .catch(
                  err => { console.log(err, 'err in account in add email'); });
        }
      });
    }
  });
      ...
      Account.findOne({'verification.email' : email}, (err, account) => {
        ...
        if (account && process.env.NODE_ENV === 'production') {
          ...
          Account.findOne({username}, (err, account) => {
            ...
            account.save(() => { setVerify({username, email, res}); });
          });
        }
      });
    }
  ...
  app.post(
      '/account/request-verification', ensureAuthenticated,
      (req, res, next) => {
  ...
  app.get('/getipintel-status', (req, res) => {
    res.send(`Current GetIPIntel counter is at ${
        getIPIntelCounter.count} - resets in ${
        (getIPIntelCounter.reset - new Date()) / 1000 / 60} minutes`);
  });
                  ...
                  if (account) {
                    ...
                    if (accountCreationDisabled.status) {
                      ...
                      Account
                          ...
                          .then(account => {
                            ...
                            if (account) {
                              ...
                              const new88 = new EightEightCounter(
                                  {date : new Date(), username});
                              new88.save(() => {
                                req.session.oauthProfile = profile;
                                res.redirect('/oauth-select-username');
                              });
                            } else if (!/^[a-z0-9]+$/i.test(profile.username) ||
                                       !profile.username ||
                          ...
                          .catch(err => { console.log(err, 'err in oauth1'); });
                    }
                  }
                })
                .catch(err => { console.log(err, 'err in oauth2'); });
          }
        })(req, res, next);
      }
    });
  ...
  app.get(
      ...
      (req, res, next) => { oAuthAuthentication(req, res, next, 'github'); });

  app.get(
      '/discord/login-callback',
      (req, res, next) => { oAuthAuthentication(req, res, next, 'discord'); });
  ...
  app.post('/oauth-select-username', (req, res, next) => {
    ...
    if (!req.session.oauthProfile || !username ||
        !/^[a-z0-9]+$/i.test(username) || username.length < 3 ||
        username.length > 12 || accountCreationDisabled.status) {
      res.status(401).send();
      return;
    }
    const ip = req.expandedIP;
    testIP(ip, (banType, unbanTime) => {
      if (banType) {
        ...
        const continueSignupConfig = {
          ...
          res,
          username,
          email : oauthType === 'discord'
                      ? oauthProfile && oauthProfile.email
                      : oauthProfile && oauthProfile._json &&
                            oauthProfile._json.email,
          signupIP : ip,
          next : continueSignup,
          isOAuth : true,
          type : oauthType,
  ...
  app.get('/revoke-discord', ensureAuthenticated, (req, res) => {
    ...
    req.user.save(() => { res.redirect('/account'); });
  });
  ...
  app.get('*', (req, res) => { renderPage(req, res, '404', '404'); });
