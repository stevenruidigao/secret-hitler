...
let modUserNames = [], editorUserNames = [], adminUserNames = [];
...
const gamesGarbageCollector = () => {
  ...
  Object.keys(games).forEach(gameName => {
    ...
    const createdTimer =
        currentGame.general && currentGame.general.timeCreated &&
        currentGame.gameState && !currentGame.gameState.isStarted &&
        new Date(currentGame.general.timeCreated.getTime() + 1000 * 60 * 10);
    const completedTimer =
        currentGame.gameState && currentGame.gameState.isCompleted &&
        currentGame.gameState.timeCompleted &&
        new Date(currentGame.gameState.timeCompleted + 1000 * 60 * 2);
    const abandonedTimer =
        currentGame.general && currentGame.general.timeAbandoned &&
        new Date(currentGame.general.timeAbandoned.getTime() + 1000 * 60 * 2);

    // To come maybe later
    // const modDeleteTimer = games[gameName].general.modDeleteDelay && new
    // Date(games[gameName].general.modDeleteDelay.getTime() + 900000);
    ...
    toDelete = (createdTimer && createdTimer < currentTime) ||
               (!games[gameName].general.modDeleteDelay && completedTimer &&
    ...
    if (toDelete && currentGame.publicPlayersState) {
      for (let affectedPlayerNumber = 0;
                ...
                .find(
                    socketId =>
                        ...
                        io.sockets.sockets[socketId]
                            ...
                            currentGame.publicPlayersState[affectedPlayerNumber]
                                .userName);
        if (!io.sockets.sockets[affectedSocketId]) {
          continue;
        }

        // I'm entirely unsure why socketio seems to misbehave with these
        // combined so often - probably just bad timing
...
const ensureInGame = (passport, game) => {
  if (game && game.publicPlayersState && game.gameState && passport &&
      passport.user) {
    const player = game.publicPlayersState.find(player => player.userName ===
                                                          passport.user);
...
const gatherStaffUsernames = () => {
  Account.find({staffRole : {$exists : true}})
      .then(accounts => {
        modUserNames =
            accounts.filter(account => account.staffRole === 'moderator')
                .map(account => account.username);
        editorUserNames =
            accounts.filter(account => account.staffRole === 'editor')
                .map(account => account.username);
        adminUserNames =
            accounts.filter(account => account.staffRole === 'admin')
                .map(account => account.username);
      })
      .catch(err => { console.log(err, 'err in finding staffroles'); });
};
    ...
    checkUserStatus(socket, () => {
      ...
      if (authenticated && passport && passport.user) {
        Account.findOne({username : passport.user}).then(account => {
          if (account.staffRole && account.staffRole.length > 0 &&
      ...
      const checkRestriction = account => {
        ...
        const warnings =
            account.warnings.filter(warning => !warning.acknowledged);
        if (warnings.length > 0) {
          const {moderator, acknowledged, ...firstWarning} =
              warnings[0]; // eslint-disable-line no-unused-vars
      ...
      if (passport && passport.user && authenticated) {
        Account.findOne({username : passport.user})
            .then(account => { isRestricted = checkRestriction(account); });
      }

      // Instantly sends the userlist as soon as the websocket is created.
      // For some reason, sending the userlist before this happens actually
      // doesn't work on the client. The event gets in, but is not used.
        ...
        if (passport.user) {
          ...
          if (dmID) {
            ...
            socket.emit('openModDMs',
                        handleAEMMessages(modDMs[dmID], passport.user,
          ...
          Account.findOne({username : username}).then(account => {
            if (account) {
              if (account.warnings && account.warnings.length > 0) {
                socket.emit('sendWarnings',
                            {username, warnings : account.warnings});
              } else {
      ...
      socket.on('feedbackForm', data => {
        ...
        if (!(data && data.feedback)) {
          socket.emit('feedbackResponse', {
            status : 'error',
            message : 'You cannot submit empty feedback.'
          });
          return;
        }
        ...
        if (data.feedback.length <= 1900) {
          Account.findOne({username : passport.user}).then(account => {
            if (!account.feedbackSubmissions)
              account.feedbackSubmissions = [];
            const newFeedback = {date : new Date(), feedback : data.feedback};

            if (account.feedbackSubmissions.length >= 2) {
              const secondMostRecentIndex =
                  account.feedbackSubmissions.length - 2;
              if (newFeedback.date -
                      account.feedbackSubmissions[secondMostRecentIndex].date >
                  1000 * 60 * 60 * 24) {
                // if it's been 24 hours since the *2nd* most recent feedback
                // submission
          ...
          socket.emit(
              'feedbackResponse',
              {status : 'error', message : 'Your feedback is too long.'});
        }
      });
      ...
      socket.on('aemOpenChat', data => {
        if (authenticated && isAEM) {
          handleOpenChat(socket, data, modUserNames, editorUserNames,
                         adminUserNames);
        }
      });
      ...
      socket.on('aemCloseChat', data => {
        if (authenticated && isAEM) {
          handleCloseChat(socket, data, modUserNames, editorUserNames,
                          adminUserNames);
        }
      });
      ...
      socket.on('aemUnsubscribeChat', data => {
        if (authenticated && isAEM) {
          handleUnsubscribeChat(socket, data, modUserNames, editorUserNames,
                                adminUserNames);
        }
      });
      ...
      socket.on('modDMsAddChat', data => {
        if (authenticated) {
          handleAddNewModDMChat(socket, passport, data, modUserNames,
      ...
      socket.on('acknowledgeWarning', () => {
        if (authenticated && isRestricted) {
          Account.findOne({username : passport.user}).then(acc => {
            acc.warnings[acc.warnings.findIndex(warning =>
      ...
      socket.on('handleUpdatedPlayerNote',
                data => { handleUpdatedPlayerNote(socket, passport, data); });

      socket.on('handleUpdatedTheme',
                data => { handleUpdatedTheme(socket, passport, data); });

      socket.on('updateModAction', data => {
        if (authenticated && isAEM) {
          handleModerationAction(socket, passport, data, false, modUserNames,
                                 editorUserNames.concat(adminUserNames));
        }
      });
      ...
      socket.on('updateTruncateGame',
                data => { handleUpdatedTruncateGame(data); });
      socket.on('addNewGameChat', data => {
        const game = findGame(data);
        if (isRestricted)
          return;
        if (authenticated) {
          handleAddNewGameChat(socket, passport, data, game, modUserNames,
                               editorUserNames, adminUserNames,
                               handleAddNewClaim, isTourneyMod);
        }
      });
      ...
      socket.on('addNewGeneralChat', data => {
        ...
        if (authenticated) {
          handleNewGeneralChat(socket, passport, data, modUserNames,
                               editorUserNames, adminUserNames);
        }
      });
      ...
      socket.on('playerReport', (data, callback) => {
        if (isRestricted || !data || !data.comment || data.comment.length > 140)
          return;
        if (authenticated) {
      ...
      socket.on('getPlayerNotes', data => { sendPlayerNotes(socket, data); });
      socket.on('getGameList', () => { sendGameList(socket); });
      socket.on('getGameInfo', uid => { sendGameInfo(socket, uid); });
      socket.on('getUserList', () => { sendUserList(socket); });
      socket.on('getGeneralChats', () => { sendGeneralChats(socket); });
      socket.on('getUserGameSettings', () => { sendUserGameSettings(socket); });
      ...
      socket.on('subscribeModChat', uid => {
        ...
        if (authenticated &&
            (isAEM || (isTourneyMod && game.general.unlistedGame))) {
          if (game && game.private && game.private.seatedPlayers) {
            const players =
                game.private.seatedPlayers.map(player => player.userName);
            Account.find({staffRole : {$exists : true, $ne : 'veteran'}})
                .then(accounts => {
                  ...
                  if (staff.length) {
                    socket.emit(
                        'sendAlert',
                        `AEM members are present: ${JSON.stringify(staff)}`);
                    return;
                  }
      ...
      socket.on('getReplayGameData',
                uid => { sendReplayGameData(socket, uid); });
      // election
      ...
      socket.on('selectedPolicies', data => {
        ...
        if (authenticated && ensureInGame(passport, game)) {
          if (game.private.lock.policyPeekAndDrop)
            selectOnePolicy(passport, game);
          else
            selectPolicies(passport, game, socket);
        }
      });
