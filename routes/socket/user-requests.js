...
module.exports.sendSpecificUserList = (socket, staffRole) => {
  // eslint-disable-line one-var
  const isAEM =
      Boolean(staffRole && staffRole !== 'altmod' && staffRole !== 'veteran');
  if (socket) {
    socket.emit('userList', {list : formattedUserList(isAEM)});
  } else {
...
const getModInfo =
    (games, users, socket, queryObj, count = 1, isTrial, isAEM) => {
      ...
      ModAction.find(queryObj)
          ...
          .then(actions => {
            const list = users.map(
                user => ({
                  status : userList
                               .find(userListUser => user.username ===
                  ...
                  email : `${user.verified ? '+' : '-'}${
                      maskEmail(user.verification.email)}`
                }));
            list.forEach(user => {
              if (user.ip && user.ip != '') {
                try {
            ...
            actions.forEach(action => {
              if (action.ip && action.ip != '') {
                if (action.ip.startsWith('-')) {
                  action.ip = 'ERROR'; // There are some bugged IPs in the list
                                       // right now, need to suppress it.
                } else {
                  try {
          ...
          .catch(err => { console.log(err, 'err in finding mod actions'); });
    };
...
module.exports.sendSignups = socket => {
  Signups.find({type : {$in : [ 'local', 'discord', 'github' ]}})
      ...
      .then(signups => { socket.emit('signupsInfo', signups); })
      .catch(err => { console.log(err, 'err in finding signups'); });
};
...
module.exports.sendUserGameSettings = socket => {
  ...
  Account.findOne({username : passport.user})
      .then(account => {
        ...
        socket.emit(
            'version',
            {current : version, lastSeen : account.lastVersionSeen || 'none'});
      })
...
module.exports.sendPlayerNotes = (socket, data) => {
  PlayerNote
      .find({userName : data.userName, notedUser : {$in : data.seatedPlayers}})
      .then(notes => {
        if (notes) {
          socket.emit('notesUpdate', notes);
        }
      })
      .catch(err => { console.log(err, 'err in getting playernotes'); });
};
...
module.exports.sendReplayGameData = (socket, uid) => {
  Game.findOne({uid}).select({_id : 0, _v : 0}).then((game, err) => {
    if (err) {
...
module.exports.sendUserReports = socket => {
  PlayerReport.find().sort({$natural : -1}).limit(500).then(reports => {
 ...
 * @param {object} socket - user socket reference.
 */
module.exports.sendGeneralChats =
    socket => { socket.emit('generalChats', generalChats); };
 ...
 */
const updateUserStatus = (module.exports.updateUserStatus = (passport, game,
                                                             override) => {
  const user = userList.find(user => user.userName === passport.user);
  if (user) {
    user.status = {
      type : override && game && !game.general.unlistedGame ? override
             : game ? game.general.private ? 'private'
                      : !game.general.unlistedGame && game.general.rainbowgame
                          ? 'rainbow'
                      : !game.general.unlistedGame ? 'playing'
                                                   : 'none'
                    : 'none',
