...
import { games, userList } from '../models.ts';
import { updateUserStatus, sendGameList, sendUserList } from '../user-requests.ts';
import { sendCommandChatsUpdate, sendInProgressGameUpdate } from '../util.ts';
...
export const handleSocketDisconnect = (socket: Socket) => {
	...
	if (passport && Object.keys(passport).length) {
		...
		// 			game.general.tournyInfo.queuedPlayers.map((player: any) => player.userName).includes(passport.user)
		// 	);
...
export const handleUserLeaveGame = (socket: Socket, game: ActiveGame, data: { isRemake: boolean }, passport: any) => {
	...
	if (
		...
		passport &&
		game.general.tournyInfo.queuedPlayers.map((player: any) => player.userName).find((name: string) => name === passport.user)
	) {
		playerLeavePretourny(game, passport.user);
	}
