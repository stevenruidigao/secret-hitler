...
import { sendUserReports, getModInfo, sendGameList, sendUserList } from '../user-requests.mjs';
import { handleDefaultIPv6Range, sendCommandChatsUpdate } from '../util.mjs';
	...
	if (!skipCheck && !data.isReportResolveChange) {
		if (!data.ip || data.ip === '') {
			...
			if (data.ip.startsWith('-')) {
				...
				data.ip = '';
				handleModerationAction(socket, passport, data, false, modUserNames, superModUserNames); // Note: Check is not skipped here, we want to still check the username.
				return;
			}
		}
	}
