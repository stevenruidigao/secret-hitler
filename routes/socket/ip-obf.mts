import { obfIP as ipv4 } from './ip-obfuscator-v4.mts';
import { obfIP as ipv6 } from './ip-obfuscator-v6.mts';

const is0to999 = (val: string) => {
...
export const expandAndSimplify = (ip: string) => {
	if (ip && ip.includes(':')) {
...
const ipToBinaryArray = (ip: string) => {
	if (ip.includes(':')) {
		return ip
			.split(':')
			.map((block: string) =>
...
export const doesIPMatchCIDR = (cidr: string, ip: string) => {
