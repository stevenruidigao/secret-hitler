...
module.exports.expandAndSimplify = ip => {
  if (ip && ip.includes(':')) {
    if (ip.startsWith('::ffff:')) {
      ...
      if (isIPv4(shortened))
        return shortened; // IPv4 embedded in IPv6
    }
    // IPv6
    const data = ip.split(':');
    const output =
...
const ipToBinaryArray = ip => {
  if (ip.includes(':')) {
    return ip.split(':')
        .map(block => parseInt(block, 16) // ipv6
                          .toString(2)
                          .padStart(16, '0'))
        .flat()
        .join('');
...
module.exports.doesIPMatchCIDR = (cidr, ip) => {
  ...
  return ipToBinaryArray(ipToMatch).substring(0, subnet) ===
         ipToBinaryArray(ip).substring(0, subnet);
};
...
module.exports.obfIP = ip => {
  ...
  if (isValidBlockCount(ip2))
    return (obfCache[ip] = ipv4(ip2));
  const res = ipv6(ip2);
  if (res == null)
    return '!!IPv6 NOT READY!!';
