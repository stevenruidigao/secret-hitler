...
module.exports.sendInProgressGameUpdate = (game, noChats) => {
  ...
  const observerSockets = roomSockets.filter(
      socket => (socket && !socket.handshake.session.passport) ||
                (socket && !seatedPlayerNames.includes(
  ...
  playerSockets.forEach(sock => {
    ...
    if (!game.gameState.isCompleted && game.gameState.isTracksFlipped) {
      const privatePlayer =
          _game.private.seatedPlayers.find(player => user === player.userName);
  ...
  if (!noChats && game.private && game.private.hiddenInfoChat &&
      game.private.hiddenInfoSubscriptions.length) {
  ...
  if (observerSockets.length) {
    observerSockets.forEach(sock => {
      const _game = Object.assign({}, game);
      const user = sock.handshake.session.passport
                       ? sock.handshake.session.passport.user
                       : null;

      if (noChats) {
        delete _game.chats;
        sock.emit('gameUpdate', secureGame(_game), true);
      } else if (user && game.private && game.private.hiddenInfoSubscriptions &&
...
module.exports.sendInProgressModChatUpdate = (game, chat, specificUser) => {
  ...
  if (roomSockets.length) {
    roomSockets.forEach(sock => {
      if (sock && sock.handshake && sock.handshake.passport &&
          sock.handshake.passport.user) {
        const {user} = sock.handshake.session.passport;
...
const getStaffRole = (user, modUserNames, editorUserNames, adminUserNames) => {
  if (modUserNames.includes(user) || newStaff.modUserNames.includes(user)) {
    return 'moderator';
  } else if (editorUserNames.includes(user) ||
             newStaff.editorUserNames.includes(user)) {
    return 'editor';
...
const handleAEMMessages =
    (dm, user, modUserNames, editorUserNames, adminUserNames) => {
...
module.exports.sendInProgressModDMUpdate =
    (dm, modUserNames, editorUserNames, adminUserNames) => {
              ...
              .sockets[Object.keys(io.sockets.sockets)
                           .find(socketId => io.sockets.sockets[socketId]
                                             ...
                                             io.sockets.sockets[socketId]
                                                     .handshake.session.passport
                                                     .user === user)]
              .emit('inProgressModDMUpdate',
                    handleAEMMessages(dm, user, modUserNames, editorUserNames,
                                      adminUserNames));
        } catch (e) {
          console.log('err', e);
        }
...
const avg = (accounts, accessor) =>
    accounts.reduce((prev, curr) => prev + accessor(curr), 0) / accounts.length;
...
module.exports.rateEloGame = (game, accounts, winningPlayerNames) => {
  ...
  const k = size * (game.general.rainbowgame
                        ? rk
                        : nk); // non-rainbow games are capped at k/r
  // Sort the players into winners and losers
  const winningAccounts =
      accounts.filter(account => winningPlayerNames.includes(account.username));
  const winningSize = winningPlayerNames.length;
  const losingAccounts = accounts.filter(
      account => !winningPlayerNames.includes(account.username));
  const losingSize = size - winningSize;
  // Construct some basic statistics for each team
  const averageRatingWinners =
      avg(winningAccounts, a => a.eloOverall || defaultELO);
  const averageRatingWinnersSeason =
      avg(winningAccounts, a => a.eloSeason || defaultELO);
  const averageRatingLosers =
      avg(losingAccounts, a => a.eloOverall || defaultELO);
  const averageRatingLosersSeason =
      avg(losingAccounts, a => a.eloSeason || defaultELO);
  // Elo Formula
  ...
  const p =
      1 /
      (1 +
       Math.pow(10, (averageRatingWinners - averageRatingLosers + bias) / 400));
  const pSeason = 1 / (1 + Math.pow(10, (averageRatingWinnersSeason -
                                         averageRatingLosersSeason + bias) /
                                            400));
  ...
  accounts.forEach(account => {
    ...
    // factor.
    const factor =
        winningPlayerNames.includes(account.username) ? winFactor : loseFactor;
...
module.exports.destroySession = username => {
  if (process.env.NODE_ENV !== 'production') {
    ...
    mongoClient.db('secret-hitler-app')
        .collection('sessions')
