...
const shufflePolicies = (module.exports.shufflePolicies = (game, isStart) => {
  ...
  if (isStart) {
    ...
    if (game.customGameSettings.trackState.lib > 0) {
      game.trackState.liberalPolicyCount =
          game.customGameSettings.trackState.lib;
    ...
    if (game.customGameSettings.trackState.fas > 0) {
      game.trackState.fascistPolicyCount =
          game.customGameSettings.trackState.fas;
  ...
  const libCount = game.customGameSettings.deckState.lib -
                   game.trackState.liberalPolicyCount;
  const fasCount = game.customGameSettings.deckState.fas -
                   game.trackState.fascistPolicyCount;
  game.private.policies =
      _.shuffle(_.range(0, libCount)
                    .map(num => 'liberal')
  ...
  if (!game.general.disableGamechat) {
    const chat = {
      ...
      chat : [
        {text : 'Deck shuffled: '},
        {text : `${libCount} liberal`, type : 'liberal'}, {text : ' and '},
        {text : `${fasCount} fascist`, type : 'fascist'}, {text : ' policies.'}
      ]
    };
...
/**
 * @param {object} game - game to act on.
 * @param {number} specialElectionPresidentIndex - number of index of the
 *     special election player (optional)
 */
module.exports.startElection = (game, specialElectionPresidentIndex) => {
  ...
  game.gameState.presidentIndex = (() => {
    const {presidentIndex, specialElectionFormerPresidentIndex} =
        game.gameState;
  ...
  game.private.summary = game.private.summary.nextTurn().updateLog({
    presidentId : game.gameState.presidentIndex,
    deckState : _.clone(game.private.policies)
  });
  ...
  sendGameList();
  game.general.status = `Election #${
      game.general.electionCount}: president to select chancellor.`;
  if (!experiencedMode && !game.general.disableGamechat) {
    pendingPresidentPlayer.gameChats.push({
      gameChat : true,
      timestamp : new Date(),
      chat : [ {text : 'You are president and must select a chancellor.'} ]
    });
  }

  pendingPresidentPlayer.playersState
      .filter((player, index) =>
                  seatedPlayers[index] && !seatedPlayers[index].isDead &&
    ...
    game.private.timerId = setTimeout(
        () => {
          if (game.gameState.timedModeEnabled) {
            ...
            selectChancellor(null, {user : pendingPresidentPlayer.userName},
                             game, {chancellorIndex});
            game.private.replayGameChats.push({
              gameChat : true,
              timestamp : new Date(),
              chat : [
                {text : pendingPresidentPlayer.userName, type : 'player'}, {
                  text :
                      ' was forced by the timer to select a random chancellor.'
                }
              ]
            });
          }
