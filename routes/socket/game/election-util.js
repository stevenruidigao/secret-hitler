...
module.exports.selectChancellor = (socket, passport, game, data,
  ...
  const seatedPlayers =
      game.private.seatedPlayers.filter(player => !player.isDead);
  ...
  if (game.publicPlayersState[chancellorIndex].isDead ||
      ...
      (chancellorIndex === game.gameState.previousElectedGovernment[0] &&
  ...
  if (!game.private.lock.selectChancellor &&
    ...
    presidentPlayer.playersState.forEach(
    ...
    game.general.status =
        `Vote on election #${game.general.electionCount} now.`;
          ...
          chat : [
            {text : 'You must vote for the election of president '}, {
              text : game.general.blindMode ? `{${presidentIndex + 1}}`
                                            : `${presidentPlayer.userName} {${
                                                  presidentIndex + 1}}`,
              type : 'player'
            },
            {text : ' and chancellor '}, {
              text : game.general.blindMode ? `{${chancellorIndex + 1}}`
                                            : `${chancellorPlayer.userName} {${
                                                  chancellorIndex + 1}}`,
              type : 'player'
            },
            {text : '.'}
    ...
    setTimeout(() => { sendInProgressGameUpdate(game); },
                ...
                () => {
                  ...
                  const activePlayerCount =
                      game.publicPlayersState
                          .filter(player => !player.leftGame || player.isDead)
                          .length;
                    ...
                    if (!game.general.disableGamechat) {
                      seatedPlayers.forEach(player => {
                        player.gameChats.push({
                          ...
                          chat : [ {
                            text :
                                'Not enough players are present, votes will not be auto-picked.'
                          } ]
                        });
                      });
                  ...
                  if (game.gameState.timedModeEnabled) {
                    const unvotedPlayerNames =
                        game.private.seatedPlayers
                            .filter(player => !player.voteStatus.hasVoted &&
                                              !player.isDead)
                    ...
                    unvotedPlayerNames.forEach(userName => {
                      selectVoting({user : userName}, game,
                      ...
                      game.private.replayGameChats.push({
                        ...
                        timestamp : new Date(),
                        chat : [
                          {text : userName, type : 'player'}, {
                            text :
                                ' was forced by the timer to select a random vote.'
                          }
                        ]
                      });
                    });
