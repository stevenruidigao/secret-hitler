...
export const assassinateMerlin = game => {
	...
	if (!game.private.lock.assassinateMerlin && game.general.avalonSH && !(game.general.isTourny && game.general.tournyInfo.isCancelled)) {
		...
		setTimeout(() => {
			...
			hitler.playersState
				.filter((player, index) => seatedPlayers[index].role.cardName === 'morgana')
				.forEach(player => {
...
export const selectPlayerToAssassinate = (passport, game, data, socket) => {
	const { seatedPlayers } = game.private;
