...
const GameSummaryBuilder =
    require('../../../models/game-summary/GameSummaryBuilder');
...
const beginGame = game => {
  ...
  if (!customGameSettings.enabled) {
    ...
    if (game.general.type == 0) {
      ...
      customGameSettings.powers =
          [ null, null, 'deckpeek', 'bullet', 'bullet' ];
      if (game.general.rebalance6p && game.publicPlayersState.length == 6)
        customGameSettings.trackState.fas = 1;
    } else if (game.general.type == 1) {
      // 7-8 players
      customGameSettings.fascistCount = 2;
      customGameSettings.hitKnowsFas = false;
      customGameSettings.powers =
          [ null, 'investigate', 'election', 'bullet', 'bullet' ];
      if (game.general.rebalance7p && game.publicPlayersState.length == 7)
        customGameSettings.deckState.fas = 10;
    } else {
      // 9-10 players
      customGameSettings.fascistCount = 3;
      customGameSettings.hitKnowsFas = false;
      customGameSettings.powers =
          [ 'investigate', 'investigate', 'election', 'bullet', 'bullet' ];
      if (game.general.rebalance9p2f && game.publicPlayersState.length == 9)
        customGameSettings.deckState.fas = 10;
    }
  }
  ...
  const roles =
      [ {cardName : 'hitler', icon : 6, team : 'fascist'} ]
          .concat(_.shuffle(
              // With custom games, up to 8 libs can be in a game, but there are
              // only 6 cards. Two are re-used in this case.
              _.range(0, 8)
                  .map(el => ({
                         cardName : 'liberal',
                         icon : el % 6,
  ...
  game.private.seatedPlayers.forEach((player, i) => {
    ...
    player.playersState =
        _.range(0, game.publicPlayersState.length).map(play => ({}));
      ...
      player.gameChats.push({
        ...
        chat : [
          {text : 'The game begins and you receive the '}, {
            text : player.role.cardName === 'hitler' ? 'hitler'
                                                     : player.role.cardName,
            type : player.role.cardName
          },
          {text : ' role and take seat '},
          {text : `#${i + 1}.`, type : 'player'}
        ]
      });
    } else {
      player.gameChats.push({
    ...
    const modOnlyChat = {
      ...
      gameChat : true,
      chat : [
        {text : `${player.userName} {${i + 1}}`, type : 'player'},
  ...
  const libPlayers = game.private.seatedPlayers.filter(
      player => player.role.team === 'liberal');
  const fasPlayers = game.private.seatedPlayers.filter(
      player => player.role.team !== 'liberal');
  ...
  Account
      .find({
        username :
            {$in : game.private.seatedPlayers.map(player => player.userName)}
      })
      .then(accounts => {
        libElo.overall =
            lib.reduce(
                (prev, curr) =>
                    (accounts.find(account => account.username === curr)
                             .eloOverall
                         ? accounts.find(account => account.username === curr)
                               .eloOverall
                         : 1600) +
  ...
  game.private.summary = new GameSummaryBuilder(
      game.general.uid, new Date(), {
        rebalance6p :
            game.general.rebalance6p && game.private.seatedPlayers.length === 6,
        rebalance7p :
            game.general.rebalance7p && game.private.seatedPlayers.length === 7,
        rebalance9p : false,
        rerebalance9p : game.general.rerebalance9p &&
                            game.private.seatedPlayers.length === 9,
  ...
  const hitlerPlayer = game.private.seatedPlayers.find(
        ...
        game.private.seatedPlayers.forEach((player, i) => {
          ...
          if (cardName === 'fascist') {
            ...
            if (customGameSettings.fascistCount == 2) {
              const otherFascist =
                  seatedPlayers.find(play => play.role.cardName === 'fascist' &&
              ...
              const otherFascists = seatedPlayers.filter(
                  play => play.role.cardName === 'fascist' &&
                          play.userName !== player.userName);
                ...
                player.gameChats.push({
                  ...
                  chat : [
                    ...
                    {text : ' in this game are '}, {
                      text : game.general.blindMode
                                 ? `{${
                                       seatedPlayers.indexOf(otherFascists[0]) +
                                       1}}`
                                 : `${otherFascists[0].userName} {${
                                       seatedPlayers.indexOf(otherFascists[0]) +
                                       1}}`,
                      type : 'player'
                    },
                    {text : ' and '}, {
                      text : game.general.blindMode
                                 ? `{${
                                       seatedPlayers.indexOf(otherFascists[1]) +
                                       1}}`
                                 : `${otherFascists[1].userName} {${
                                       seatedPlayers.indexOf(otherFascists[1]) +
                                       1}}`,
                      type : 'player'
                    },
              ...
              otherFascists.forEach(fascistPlayer => {
                player.playersState[seatedPlayers.indexOf(fascistPlayer)]
                    .nameStatus = 'fascist';
              });
              otherFascists.forEach(fascistPlayer => {
                player.playersState[seatedPlayers.indexOf(fascistPlayer)]
                    .notificationStatus = 'fascist';
              });
            }
            ...
            const chat = {
              ...
              gameChat : true,
              chat : [
                {text : 'You see that '}, {text : 'hitler', type : 'hitler'},
                {text : ' in this game is '}, {
                  text : game.general.blindMode
                             ? `{${seatedPlayers.indexOf(hitlerPlayer) + 1}}`
                             : `${hitlerPlayer.userName} {${
                                   seatedPlayers.indexOf(hitlerPlayer) + 1}}`,
                  type : 'player'
                }
              ]
            ...
            if (!game.general.disableGamechat) {
              if (customGameSettings.hitKnowsFas) {
                chat.chat.push({text : '. They also see that you are a '},
                               {text : 'fascist', type : 'fascist'},
                               {text : '.'});
              } else {
                chat.chat.push({text : '. They do not know you are a '},
                               {text : 'fascist', type : 'fascist'},
                               {text : '.'});
              }
            ...
            player.playersState[seatedPlayers.indexOf(hitlerPlayer)]
                .notificationStatus = 'hitler';
            player.playersState[seatedPlayers.indexOf(hitlerPlayer)]
                .nameStatus = 'hitler';
            ...
            if (customGameSettings.hitKnowsFas) {
              if (customGameSettings.fascistCount == 1) {
                ...
                player.playersState[seatedPlayers.indexOf(otherFascist)]
                    .nameStatus = 'fascist';
                player.playersState[seatedPlayers.indexOf(otherFascist)]
                    .notificationStatus = 'fascist';
              } else {
                    ...
                    chat : [
                      ...
                      {text : customGameSettings.fascistCount == 3 ? ', ' : ''},
                      {
                        text : customGameSettings.fascistCount == 3
                                   ? game.general.blindMode
                                         ? `{${
                                               seatedPlayers.indexOf(
                                                   otherFascists[2]) +
                                               1}}`
                                         : `${otherFascists[2].userName} {${
                                               seatedPlayers.indexOf(
                                                   otherFascists[2]) +
                                               1}}`
                                   : '',
                      ...
                      {
                        text : customGameSettings.fascistCount == 3 ? ', and'
                                                                    : ' and '
                      },
                      {
                        text :
              ...
              if (!game.general.disableGamechat) {
                player.gameChats.push({
                  ...
                  chat : [
                    {
                      text : `There ${
                          customGameSettings.fascistCount == 1 ? 'is' : 'are'} `
                    },
                    {
                      text : customGameSettings.fascistCount == 1 ? '1 fascist'
                             : customGameSettings.fascistCount == 2
                                 ? '2 fascists'
                                 : '3 fascists',
                      type : 'fascist'
  ...
  setTimeout(() => {
    game.private.seatedPlayers.forEach((player, i) => {
      ...
      player.playersState.forEach(play => { play.notificationStatus = ''; });
    });
  ...
  setTimeout(
      () => {
        game.publicPlayersState.forEach(
            player => { player.cardStatus.cardDisplayed = false; });
        sendInProgressGameUpdate(game, true);
      },
      process.env.NODE_ENV === 'development' ? 100
      : experiencedMode                      ? 5200
                                             : 7000);

  setTimeout(
      () => {
        game.private.seatedPlayers.forEach(player => {
          player.playersState.forEach(state => { state.cardStatus = {}; });
        });
        game.gameState.presidentIndex = -1;
        startElection(game);
      },
      process.env.NODE_ENV === 'development' ? 100
      : experiencedMode                      ? 5400
                                             : 9000);
  ...
  for (let affectedPlayerNumber = 0;
    ...
    if (process.env.NODE_ENV !== 'development') {
      io.sockets.sockets[affectedSocketId].emit(
          'pingPlayer', 'Secret Hitler IO: The game has started!');
    }
  }
};
...
module.exports = game => {
  ...
  const countDown = setInterval(() => {
    if (!startGamePause) {
      ...
      game.general.status = `Game starts in ${startGamePause} second${
          startGamePause === 1 ? '' : 's'}.`;
      sendInProgressGameUpdate(game, true);
      startGamePause--;
    }
  ...
  game.general.type = game.general.playerCount < 7 ? 0
                      : game.general.playerCount < 9
                          ? 1
                          : 2; // different fascist tracks
