const {sendInProgressGameUpdate, sendInProgressModChatUpdate} =
    require('../util');
...
const powerMapping = {
  investigate : [
    investigateLoyalty,
    'The president must investigate the party membership of another player.'
  ],
  deckpeek : [ policyPeek, 'The president must examine the top 3 policies.' ],
  election : [
    specialElection,
    'The president must select a player for a special election.'
  ],
  bullet :
      [ executePlayer, 'The president must select a player for execution.' ],
  reverseinv : [
    showPlayerLoyalty,
    'The president must reveal their party membership to another player.'
  ],
  peekdrop : [
    policyPeekAndDrop,
    'The president must examine the top policy, and may discard it.'
  ]
};
...
const enactPolicy = (game, team, socket) => {
  ...
  game.private.summary = game.private.summary.updateLog({enactedPolicy : team});
  ...
  game.trackState.enactedPolicies.push(
      {position : 'middle', cardBack : team, isFlipped : false});
  ...
  setTimeout(
      () => {
        ...
        game.gameState.audioCue =
            team === 'liberal' ? 'enactPolicyL' : 'enactPolicyF';
        sendInProgressGameUpdate(game, true);
      },
      ...
      () => {
            ...
            },
            {
              text : ` policy has been enacted. (${
                  team === 'liberal'
                      ...
                      : game.trackState.fascistPolicyCount.toString()}/${
                  team === 'liberal' ? '5' : '6'})`
            }
          ]
        };
        const addPreviousGovernmentStatus = () => {
          ...
          if (game.trackState.electionTrackerCount <= 2 &&
              game.publicPlayersState.findIndex(
                  player => player.governmentStatus === 'isChancellor') > -1) {
            game.publicPlayersState[game.gameState.presidentIndex]
                .previousGovernmentStatus = 'wasPresident';
            game
                .publicPlayersState[game.publicPlayersState.findIndex(
                    player => player.governmentStatus === 'isChancellor')]
                .previousGovernmentStatus = 'wasChancellor';
          }
        };
        const powerToEnact =
            team === 'fascist'
                ? game.customGameSettings.enabled
                      ? powerMapping[game.customGameSettings.powers
                                         [game.trackState.fascistPolicyCount -
                                          1]]
                      : presidentPowers[game.general
                                            .type][game.trackState
                                                       .fascistPolicyCount -
                                                   1]
                : null;

        game.trackState.enactedPolicies[index].position =
            team === 'liberal' ? `liberal${game.trackState.liberalPolicyCount}`
                               : `fascist${game.trackState.fascistPolicyCount}`;

        if (!game.general.disableGamechat) {
          game.private.seatedPlayers.forEach(
              player => { player.gameChats.push(chat); });

          game.private.unSeatedGameChats.push(chat);
        }

        if (game.trackState.liberalPolicyCount === 5 ||
            game.trackState.fascistPolicyCount === 6) {
          ...
          game.gameState.audioCue = game.trackState.liberalPolicyCount === 5
                                        ? 'liberalsWin'
                                        : 'fascistsWin';
          setTimeout(() => {
            game.publicPlayersState.forEach(
                (player, i) => { player.cardStatus.isFlipped = true; });
            game.gameState.audioCue = '';
            completeGame(game, game.trackState.liberalPolicyCount === 5
                                   ? 'liberal'
                                   : 'fascist');
          ...
          if (!game.general.disableGamechat) {
            seatedPlayers.forEach(player => { player.gameChats.push(chat); });
            ...
            game.private.timerId = setTimeout(
                () => {
                  if (game.gameState.timedModeEnabled) {
                    ...
                    let list = seatedPlayers.filter(
                        (player, i) =>
                            i !== presidentIndex && !seatedPlayers[i].isDead);
                    ...
                    case 'The president must examine the top 3 policies.':
                      ...
                      game.private.replayGameChats.push({
                        ...
                        timestamp : new Date(),
                        chat : [
                          {text : president.userName, type : 'player'},
                    ...
                    case 'The president must select a player for execution.':
                      if (president.role.cardName === 'fascist') {
                        list = list.filter(player => player.role.cardName !==
                                                     'hitler');
                      }
                      selectPlayerToExecute({user : president.userName}, game, {
                      ...
                      game.private.replayGameChats.push({
                        ...
                        chat : [
                          {text : president.userName, type : 'player'}, {
                            text :
                                ' was forced by the timer to select a random player to execute.'
                          }
                        ]
                      });
                      break;
                    case 'The president must investigate the party membership of another player.':
                      selectPartyMembershipInvestigate(
                          {user : president.userName}, game, {
                            playerIndex :
                                seatedPlayers.indexOf(_.shuffle(list)[0])
                          },
                          socket);
                      game.private.replayGameChats.push({
                        gameChat : true,
                        ...
                        chat : [
                          {text : president.userName, type : 'player'}, {
                            text :
                                ' was forced by the timer to select a random player to investigate.'
                          }
                        ]
                      });
                      break;
                    case 'The president must select a player for a special election.':
                      ...
                      game.private.replayGameChats.push({
                        ...
                        chat : [
                          {text : president.userName, type : 'player'}, {
                            text :
                                ' was forced by the timer to select a random player to special elect.'
                          }
                        ]
                      });
                      break;
                ...
                process.env.DEVTIMEDDELAY ? process.env.DEVTIMEDDELAY
                                          : game.general.timedMode * 1000);
            sendInProgressGameUpdate(game);
          }
        } else {
          sendInProgressGameUpdate(game);
...
const selectPresidentVoteOnVeto = (passport, game, data, socket) => {
  ...
  const chancellorIndex = game.publicPlayersState.findIndex(
      player => player.governmentStatus === 'isChancellor');
  const publicChancellor = game.publicPlayersState[chancellorIndex];
  const publicPresident =
      game.publicPlayersState[game.gameState.presidentIndex];
  ...
  if (game.general.isRemade) {
    if (socket) {
      socket.emit('sendAlert',
                  'This game has been remade and is now no longer playable.');
    }
    return;
  }
  ...
  if (!game.private.lock.selectPresidentVoteOnVeto &&
      ...
      president.cardFlingerState && president.cardFlingerState[0]) {
    ...
    president.cardFlingerState[0].action =
        president.cardFlingerState[1].action = '';
    president.cardFlingerState[0].cardStatus.isFlipped =
        president.cardFlingerState[1].cardStatus.isFlipped = false;
    ...
    publicPresident.cardStatus = {
      cardDisplayed : true,
      cardFront : 'ballot',
      cardBack : {cardName : data.vote ? 'ja' : 'nein'}
    };
        ...
        () => {
            ...
            chat : [
              {text : 'President '}, {
                text : game.general.blindMode
                           ? `{${
                                 game.private.seatedPlayers.indexOf(president) +
                                 1}}`
                           : `${passport.user} {${
                                 game.private.seatedPlayers.indexOf(president) +
                                 1}}`,
              ...
              {
                text : data.vote ? ' has voted to veto this election.'
          ...
          if (data.vote) {
            ...
            const chat = {
              ...
              chat : [ {
                text :
                    `The President and Chancellor have voted to veto this election and the election tracker moves forward. (${
            ...
            setTimeout(
                () => {
                  ...
                  if (game.trackState.electionTrackerCount <= 2 &&
                      game.publicPlayersState.findIndex(
                          player => player.governmentStatus ===
                                    'isChancellor') > -1) {
                    game.publicPlayersState.forEach(player => {
                      if (player.previousGovernmentStatus) {
                        player.previousGovernmentStatus = '';
            ...
            setTimeout(
                () => {
                  ...
                  enactPolicy(game, game.private.currentElectionPolicies[0],
                              socket);
                  setTimeout(() => {
                    publicChancellor.cardStatus.isFlipped =
...
const selectChancellorVoteOnVeto = (passport, game, data, socket) => {
  ...
  const chancellorIndex = game.publicPlayersState.findIndex(
      player => player.governmentStatus === 'isChancellor');
  const chancellor = game.private.seatedPlayers.find(
      player => player.userName === game.private._chancellorPlayerName);
  const publicChancellor = game.publicPlayersState[chancellorIndex];
  ...
  if (!publicChancellor || !publicChancellor.userName ||
      passport.user !== publicChancellor.userName) {
    return;
  }
  ...
  if (!game.private.lock.selectChancellorVoteOnVeto && chancellor &&
      chancellor.cardFlingerState && chancellor.cardFlingerState.length &&
      game.publicPlayersState[chancellorIndex] &&
      !(game.general.isTourny && game.general.tournyInfo.isCancelled)) {
    ...
    chancellor.cardFlingerState[0].action =
        chancellor.cardFlingerState[1].action = '';
    chancellor.cardFlingerState[0].cardStatus.isFlipped =
        chancellor.cardFlingerState[1].cardStatus.isFlipped = false;
        ...
        () => {
          const chat = {
            ...
            chat : [
              {text : 'Chancellor '}, {
                text : game.general.blindMode
                           ? `{${chancellorIndex + 1}}`
                           : `${passport.user} {${chancellorIndex + 1}}`,
                type : 'player'
              },
              {
                text : data.vote ? ' has voted to veto this election.'
          ...
          if (data.vote) {
            president.cardFlingerState = [
              {
                ...
                action : 'active',
                cardStatus :
                    {isFlipped : false, cardFront : 'ballot', cardBack : 'ja'}
              },
              {
                ...
                notificationStatus : '',
                cardStatus :
                    {isFlipped : false, cardFront : 'ballot', cardBack : 'nein'}
              }
            ];
            ...
            setTimeout(
                () => {
                  president.cardFlingerState[0].cardStatus.isFlipped =
                      president.cardFlingerState[1].cardStatus.isFlipped = true;
                  president.cardFlingerState[0].notificationStatus =
                      president.cardFlingerState[1].notificationStatus =
                          'notification';
                          ...
                          if (game.gameState.timedModeEnabled) {
                            ...
                            game.private.replayGameChats.push({
                              ...
                              chat : [
                                {text : president.userName, type : 'player'}, {
                                  text :
                                      ' was forced by the timer to select a random veto vote.'
                                }
                              ]
                            });
                          }
                        },
                        process.env.DEVTIMEDDELAY
                            ? process.env.DEVTIMEDDELAY
                            : game.general.timedMode * 1000);
                  }
                },
...
const selectChancellorPolicy = (passport, game, data, wasTimer, socket) => {
  const {experiencedMode} = game.general;
  const presidentIndex = game.publicPlayersState.findIndex(
      player => player.governmentStatus === 'isPresident');
  const president = game.private.seatedPlayers[presidentIndex];
  const chancellorIndex = game.publicPlayersState.findIndex(
      player => player.governmentStatus === 'isChancellor');
  const chancellor = game.private.seatedPlayers[chancellorIndex];
  const enactedPolicy =
      game.private.currentChancellorOptions[data.selection === 3 ? 1 : 0];
  ...
  if (!game.private.lock.selectChancellorPolicy && chancellor &&
      chancellor.cardFlingerState && chancellor.cardFlingerState.length &&
      !(game.general.isTourny && game.general.tournyInfo.isCancelled)) {
    if (!wasTimer && !game.general.private) {
      if (chancellor.role.team === 'liberal' && enactedPolicy === 'fascist' &&
      ...
      if (chancellor.role.team === 'fascist' && enactedPolicy === 'liberal' &&
          game.trackState.liberalPolicyCount >= 4 &&
          (game.private.currentChancellorOptions[0] === 'fascist' ||
           game.private.currentChancellorOptions[1] === 'fascist')) {
        ...
        makeReport({
          ...
          role : chancellor.role.cardName,
          situation :
              `was given choice as chancellor with 4 blues on the track, and played liberal.`,
          election : game.general.electionCount,
          title : game.general.name,
          uid : game.general.uid,
          gameType : game.general.casualGame     ? 'Casual'
    ...
    const modOnlyChat = {
      ...
      chat : [
        ...
        {
          text : wasTimer ? ' has automatically chosen to play a '
                          : ' has chosen to play a '
        },
        {text : enactedPolicy, type : enactedPolicy},
      ...
      setTimeout(
          () => {
            ...
            setTimeout(
                () => {
                  chancellor.cardFlingerState[0].cardStatus.isFlipped =
                      chancellor.cardFlingerState[1].cardStatus.isFlipped =
                          true;
                  chancellor.cardFlingerState[0].notificationStatus =
                      chancellor.cardFlingerState[1].notificationStatus =
                          'notification';
                        ...
                        () => {
                          if (game.gameState.timedModeEnabled) {
                            ...
                            game.private.replayGameChats.push({
                              ...
                              timestamp : new Date(),
                              chat : [
                                {text : chancellor.userName, type : 'player'}, {
                                  text :
                                      ' was forced by the timer to select a random veto vote.'
                                }
                              ]
...
const selectPresidentPolicy = (passport, game, data, wasTimer, socket) => {
  ...
  const chancellorIndex = game.publicPlayersState.findIndex(
      player => player.governmentStatus === 'isChancellor');
  const chancellor = game.private.seatedPlayers[chancellorIndex];
  const nonDiscardedPolicies =
      _.range(0, 3).filter(num => num !== data.selection);
  ...
  if (!president || president.userName !== passport.user ||
      nonDiscardedPolicies.length !== 2) {
    return;
  }

  if (!game.private.lock.selectPresidentPolicy && president &&
      president.cardFlingerState && president.cardFlingerState.length &&
      Number.isInteger(chancellorIndex) &&
      game.publicPlayersState[chancellorIndex] &&
      !(game.general.isTourny && game.general.tournyInfo.isCancelled)) {
    if (game.general.timedMode && game.private.timerId) {
    ...
    const modOnlyChat = {
      ...
      chat : [
        ...
        {
          text : wasTimer ? ' has automatically discarded a '
                          : ' has chosen to discard a '
        },
        {text : discarded, type : discarded},
        {text : wasTimer ? ' policy due to the timer expiring.' : ' policy.'}
      ]
    };
    game.private.hiddenInfoChat.push(modOnlyChat);
    ...
    if (!wasTimer && !game.general.private) {
      // const presGetsPower =
      // presidentPowers[game.general.type][game.trackState.fascistPolicyCount]
      // ? true : false;
          ...
          if (track4blue) {
            if (passedNicer === 'RR') {
              ...
              makeReport({
                ...
                situation :
                    `got BRR with 4 blues on the track, and tossed the blue.`,
                election : game.general.electionCount,
                title : game.general.name,
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
              ...
              makeReport({
                ...
                role : president.role.cardName,
                situation :
                    `got BBR with 4 blues on the track, and did not force.`,
                election : game.general.electionCount,
                title : game.general.name,
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
            ...
            if (passedNicer === 'RR') {
              ...
              makeReport({
                ...
                role : president.role.cardName,
                situation :
                    `got BBR during veto zone, and did not force 5th blue.`,
                election : game.general.electionCount,
                title : game.general.name,
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
          ...
          if (track4blue) {
            if (passedNicer === 'BB' && chancellor.role.team !== 'liberal') {
              ...
              makeReport({
                ...
                role : president.role.cardName,
                situation :
                    `got BBR with 4 blues on the track, and forced blues on a fascist chancellor.`,
                election : game.general.electionCount,
                title : game.general.name,
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
              ...
              makeReport({
                ...
                role : president.role.cardName,
                situation :
                    `got BRR with 4 blues on the track, and offered choice to a liberal chancellor.`,
                election : game.general.electionCount,
                title : game.general.name,
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
            ...
            if (passedNicer === 'BB' && chancellor.role.team !== 'liberal') {
              ...
              makeReport({
                ...
                role : president.role.cardName,
                situation :
                    `got BBR with 5 reds on the track, and forced blues on a fascist chancellor.`,
                election : game.general.electionCount,
                title : game.general.name,
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
              ...
              makeReport({
                ...
                role : president.role.cardName,
                situation :
                    `got BRR with 5 reds on the track, and offered choice to a liberal chancellor.`,
                election : game.general.electionCount,
                title : game.general.name,
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
    ...
    game.private.summary = game.private.summary.updateLog({
      chancellorHand : game.private.currentElectionPolicies.filter(
          (p, i) => i !== data.selection)
    });
    ...
    president.cardFlingerState[0].action =
        president.cardFlingerState[1].action =
            president.cardFlingerState[2].action = '';
    president.cardFlingerState[0].cardStatus.isFlipped =
        president.cardFlingerState[1].cardStatus.isFlipped =
            president.cardFlingerState[2].cardStatus.isFlipped = false;
    ...
    chancellor.cardFlingerState = [
      {
        ...
        cardStatus : {
          ...
          cardFront : 'policy',
          cardBack : `${
              game.private.currentElectionPolicies[nonDiscardedPolicies[0]]}p`
        }
      },
      {
        position : 'middle-right',
        ...
        cardStatus : {
          ...
          cardFront : 'policy',
          cardBack : `${
              game.private.currentElectionPolicies[nonDiscardedPolicies[1]]}p`
        }
      }
    ];
    ...
    setTimeout(() => {
      president.cardFlingerState = [];
      chancellor.cardFlingerState.forEach(
          cardFlinger => { cardFlinger.cardStatus.isFlipped = true; });
      chancellor.cardFlingerState.forEach(
          cardFlinger => { cardFlinger.notificationStatus = 'notification'; });
        ...
        game.private.timerId = setTimeout(
            () => {
              if (game.gameState.timedModeEnabled) {
                ...
                selectChancellorPolicy({user : chancellor.userName}, game,
                ...
                game.private.replayGameChats.push({
                  ...
                  chat : [
                    {text : chancellor.userName, type : 'player'}, {
                      text :
                          ' was forced by the timer to select a random policy to enact.'
                    }
                  ]
                });
              }
...
module.exports.selectVoting = (passport, game, data, socket, force = false) => {
  ...
  const {experiencedMode} = game.general;
  const player =
      seatedPlayers.find(player => player.userName === passport.user);
  const playerIndex =
      seatedPlayers.findIndex(play => play.userName === passport.user);
  ...
  const passedElection = socket => {
    ...
    game.private._chancellorPlayerName =
        game.private.seatedPlayers[chancellorIndex].userName;

    if (game.gameState.previousElectedGovernment.length) {
      game.private.seatedPlayers[game.gameState.previousElectedGovernment[0]]
          .playersState[game.gameState.previousElectedGovernment[0]]
          .claim = '';
      game.private.seatedPlayers[game.gameState.previousElectedGovernment[1]]
          .playersState[game.gameState.previousElectedGovernment[1]]
          .claim = '';
      let affectedSocketId =
    ...
    game.private.currentElectionPolicies = [
      game.private.policies.shift(), game.private.policies.shift(),
    ...
    if (!verifyCorrect(game.private.currentElectionPolicies[0]) ||
    ...
    const modOnlyChat = {
      ...
      chat : [
        ...
        {text : ' received '}, {
          text : game.private.currentElectionPolicies[0] === 'liberal' ? 'B'
                                                                       : 'R',
          type : game.private.currentElectionPolicies[0]
        },
        {
          text : game.private.currentElectionPolicies[1] === 'liberal' ? 'B'
                                                                       : 'R',
          type : game.private.currentElectionPolicies[1]
        },
        {
          text : game.private.currentElectionPolicies[2] === 'liberal' ? 'B'
                                                                       : 'R',
    ...
    setTimeout(() => {
      seatedPlayers[presidentIndex].cardFlingerState[0].cardStatus.isFlipped =
          seatedPlayers[presidentIndex]
              .cardFlingerState[1]
              .cardStatus.isFlipped = seatedPlayers[presidentIndex]
                                          .cardFlingerState[2]
                                          .cardStatus.isFlipped = true;
      ...
      game.gameState.previousElectedGovernment =
          [ presidentIndex, chancellorIndex ];
            ...
            () => {
              if (game.gameState.timedModeEnabled) {
                ...
                selectPresidentPolicy(
                    {user : seatedPlayers[presidentIndex].userName}, game,
                    {selection : Math.floor(Math.random() * 3)}, true, socket);
                game.private.replayGameChats.push({
                  gameChat : true,
                  timestamp : new Date(),
                  chat : [
                    ...
                    {
                      text :
                          ' was forced by the timer to select a random policy to discard.'
                    }
                  ]
                });
              }
  ...
  const failedElection = () => {
    ...
    if (game.trackState.electionTrackerCount >= 3) {
      const chat = {
        ...
        chat : [ {
          text :
              'The third consecutive election has failed and the top policy is enacted.'
        } ]
      };
            ...
            setTimeout(
                () => {
                  if (game.gameState.timedModeEnabled &&
                    ...
                    const chancellorIndex =
                        _.shuffle(game.gameState.clickActionInfo[1])[0];
                    ...
                    game.private.replayGameChats.push({
								...
								chat: [
									{
										text: game.private.seatedPlayers[game.gameState.presidentIndex].userName,
										type: 'player'
									},
									{
      ...
      setTimeout(() => { module.exports.startElection(game); },
                 process.env.NODE_ENV === 'development' ? 100
                 : experiencedMode                      ? 500
                                                        : 2000);
    }
  ...
  const flipBallotCards = socket => {
    ...
    const isConsensus =
        game.publicPlayersState.filter(player => !player.isDead)
            .every((el, i) => (seatedPlayers[i]
                                   ? seatedPlayers[i].voteStatus.didVoteYes ===
                                         seatedPlayers[0].voteStatus.didVoteYes
                                   : false));

    game.publicPlayersState.forEach((player, i) => {
      if (!player.isDead && seatedPlayers[i]) {
        player.cardStatus.cardBack.cardName =
            seatedPlayers[i].voteStatus.didVoteYes ? 'ja' : 'nein';
    ...
    setTimeout(
        () => {
          const chat = {timestamp : new Date(), gameChat : true};

          game.publicPlayersState.forEach(
              (play, i) => { play.cardStatus.cardDisplayed = false; });

          setTimeout(
              () => {
                game.publicPlayersState.forEach(
                    (play, i) => { play.cardStatus.isFlipped = false; });
                sendInProgressGameUpdate(game);
              },
              process.env.NODE_ENV === 'development' ? 100
              : experiencedMode                      ? 500
          ...
          if (seatedPlayers
                      .filter(play =>
                                  play.voteStatus.didVoteYes && !play.isDead)
                      .length /
            ...
            game.publicPlayersState[chancellorIndex].governmentStatus =
                'isChancellor';
            ...
            if (game.trackState.fascistPolicyCount >=
                    game.customGameSettings.hitlerZone &&
                game.private.seatedPlayers[chancellorIndex].role.cardName ===
                    'hitler') {
              ...
              const chat = {
                ...
                gameChat : true,
                chat : [
                  {text : 'Hitler', type : 'hitler'}, {
                    text : ` has been elected chancellor after the ${
                        getNumberText(
                            game.customGameSettings
                                .hitlerZone)} fascist policy has been enacted.`
                  }
                ]
              };
              ...
              setTimeout(
                  () => {
                    game.gameState.audioCue = '';
                    game.publicPlayersState.forEach(
                        player => { player.cardStatus.isFlipped = true; });
                    completeGame(game, 'fascist');
                  },
                  process.env.NODE_ENV === 'development' ? 100
                  : experiencedMode                      ? 2000
                                                         : 4000);
            } else {
              ...
              chat.chat = [ {
                text :
                    `The election fails and the election tracker moves forward. (${
                        game.trackState.electionTrackerCount + 1}/3)`
              } ];
        ...
        process.env.NODE_ENV === 'development' ? 2100
        : isConsensus                          ? 1500
                                               : 6000);
  };
  ...
  if (seatedPlayers.length !== seatedPlayers
                                   .filter(play => play && play.voteStatus &&
                                                   play.voteStatus.hasVoted)
                                   .length &&
      player && player.voteStatus) {
    player.voteStatus.hasVoted = !player.voteStatus.hasVoted    ? true
                                 : player.voteStatus.didVoteYes ? !data.vote
                                                                : data.vote;
    player.voteStatus.didVoteYes =
        player.voteStatus.hasVoted ? data.vote : false;
    ...
    if (data.vote) {
      player.cardFlingerState = [
        {
          position : 'middle-left',
          notificationStatus : !player.voteStatus.hasVoted ? 'notification'
                                                           : 'selected',
          action : 'active',
        ...
        },
        {
          ...
          action : 'active',
          cardStatus :
              {isFlipped : true, cardFront : 'ballot', cardBack : 'nein'}
        }
      ];
    } else {
      player.cardFlingerState = [
    ...
    if (seatedPlayers.filter(play => play.voteStatus.hasVoted && !play.isDead)
            .length === game.general.livingPlayerCount) {
      game.general.status = 'Tallying results of ballots..';
      seatedPlayers.forEach(player => {
        if (player.cardFlingerState.length) {
          player.cardFlingerState[0].action =
              player.cardFlingerState[1].action = '';
          player.cardFlingerState[0].action =
              player.cardFlingerState[1].action = '';
          player.cardFlingerState[0].cardStatus.isFlipped =
              player.cardFlingerState[1].cardStatus.isFlipped = false;
        }
      });
      sendInProgressGameUpdate(game, true);
      setTimeout(() => {
        seatedPlayers.forEach(player => { player.cardFlingerState = []; });
        sendInProgressGameUpdate(game, true);
      }, experiencedMode ? 200 : 2000);
      setTimeout(
          () => {
