...
const buildEnhancedGameSummary =
    require('../../../models/game-summary/buildEnhancedGameSummary');
...
const {CURRENTSEASONNUMBER} =
    require('../../../src/frontend-scripts/node-constants.js');

const generateGameObject = game => {
  const casualBool = Boolean(
      game.general.casualGame); // Because Mongo is explicitly typed and
                                // integers are not truthy according to it
  ...
  if (game.gameState && game.gameState.isCompleted) {
    return {
      ...
      chats : game.chats.concat(game.private.unSeatedGameChats)
                  .concat(game.private.replayGameChats),
      isVerifiedOnly : game.general.isVerifiedOnly,
      season : CURRENTSEASONNUMBER,
      winningPlayers :
          game.private.seatedPlayers.filter(player => player.wonGame)
              .map(player => ({
      ...
      isTournyFirstRound :
          game.general.isTourny && game.general.tournyInfo.round === 1,
      isTournySecondRound :
          game.general.isTourny && game.general.tournyInfo.round === 2,
 ...
 */
const saveGame = game => {
  const summary = game.gameState.isCompleted && game.private.summary &&
                  game.private.summary.publish();
...
// Save a game and then potentially perform another action (usually deleting the
// game)
const saveOrUpdateGame = (gameID, callback) => {
  ...
  Game.findOne({uid : gameID}).then(game => {
    if (game) {
      const newObject = generateGameObject(
          gameInMemory); // in theory this should only be chats (as the only
                         // time a game is saved and *not* deleted is on game
                         // end) but for forwards compatibility all keys are
                         // checked

      for (const key in newObject) {
        if (newObject.hasOwnProperty(key) && game[key] !== newObject[key]) {
          // check in order to prevent unnecessarily marking fields as modified
          // in mongoose
...
module.exports.completeGame = (game, winningTeamName) => {
  if (game && game.unsentReports) {
    game.unsentReports.forEach(report => {
      makeReport({...report}, game,
                 report.type === 'modchat' ? 'modchatdelayed'
                                           : 'reportdelayed');
    });
    game.unsentReports = [];
  }
  ...
  for (let affectedPlayerNumber = 0;
       affectedPlayerNumber < game.publicPlayersState.length;
  ...
  const winningPlayerNames =
      winningPrivatePlayers.map(player => player.userName);
  ...
  game.general.status = winningTeamName === 'fascist'
  ...
  seatedPlayers.forEach(
      player => { player.gameChats.push(chat, remainingPoliciesChat); });
  ...
  if (!game.general.private && !game.general.casualGame &&
      ...
      !game.general.practiceGame && !game.general.unlistedGame) {
    Account
        .find({username : {$in : seatedPlayers.map(player => player.userName)}})
        .then(results => {
          const isRainbow = game.general.rainbowgame;
          const isTournamentFinalGame =
              game.general.isTourny && game.general.tournyInfo.round === 2;
          ...
          seatedPlayers = [
            ...seatedPlayers.filter(e => e.role.cardName === 'hitler')
                .sort(byUsername),
            ...seatedPlayers.filter(e => e.role.cardName === 'fascist')
                .sort(byUsername),
            ...seatedPlayers.filter(e => e.role.cardName === 'liberal')
                .sort(byUsername)
          ];

          results.forEach(player => {
            const listUser =
                userList.find(user => user.userName === player.username);
            if (listUser) {
            ...
            const seatedPlayer =
                seatedPlayers.find(p => p.userName === player.username);
            seatedPlayers.forEach((eachPlayer, i) => {
              const playerChange = eloAdjustments[eachPlayer.userName];
              const activeChange = player.gameSettings.disableSeasonal
                                       ? playerChange.change
                                       : playerChange.changeSeason;
              if (!player.gameSettings.disableElo) {
                seatedPlayer.gameChats.push({
                  ...
                  chat : [
                    ...
                    {text : `'s Elo: `},
                    {text : ` ${activeChange > 0 ? '+' : '-'}`},
                    {text : Math.abs(activeChange).toFixed(1), type : 'player'}
                  ]
                });
              }
            });
            ...
            if (winningPlayerNames.includes(player.username)) {
              if (isRainbow) {
                player.rainbowWins =
                    player.rainbowWins ? player.rainbowWins + 1 : 1;
              ...
              if (isTournamentFinalGame && !game.general.casualGame) {
                ...
                io.sockets.sockets[playerSocketId].emit('gameSettings',
                                                        player.gameSettings);
              }
            } else {
              if (isRainbow) {
                player.rainbowLosses =
                    player.rainbowLosses ? player.rainbowLosses + 1 : 1;
                player[`rainbowLossesSeason${CURRENTSEASONNUMBER}`] =
                    player[`rainbowLossesSeason${CURRENTSEASONNUMBER}`]
                        ? player[`rainbowLossesSeason${CURRENTSEASONNUMBER}`] +
                              1
                ...
                if (winner) {
                  if (isRainbow) {
                    userEntry.rainbowWins =
                        userEntry.rainbowWins ? userEntry.rainbowWins + 1 : 1;
                    userEntry.rainbowLosses =
                        userEntry.rainbowLosses ? userEntry.rainbowLosses : 0;
                    userEntry[`rainbowWinsSeason${CURRENTSEASONNUMBER}`] =
                        userEntry[`rainbowWinsSeason${CURRENTSEASONNUMBER}`]
                            ? userEntry[`rainbowWinsSeason${
                                  CURRENTSEASONNUMBER}`] +
                                  1
                            : 1;
                    userEntry[`rainbowLossesSeason${CURRENTSEASONNUMBER}`] =
                        userEntry[`rainbowLossesSeason${CURRENTSEASONNUMBER}`]
                            ? userEntry[`rainbowWinsSeason${
                                  CURRENTSEASONNUMBER}`]
                            : 0;
  ...
  if (game.general.isTourny) {
    if (game.general.tournyInfo.round === 1) {
      ...
      const otherUid = tableUidLastLetter === 'A'
                           ? `${uid.substr(0, uid.length - 1)}B`
                           : `${uid.substr(0, uid.length - 1)}A`;
      ...
      if (!otherGame || otherGame.gameState.isCompleted) {
        ...
        const countDown = setInterval(() => {
          if (gamePause) {
            game.general.status = `Final game starts in ${gamePause} ${
                gamePause === 1 ? 'second' : 'seconds'}.`;
            if (otherGame) {
              otherGame.general.status = `Final game starts in ${gamePause} ${
                  gamePause === 1 ? 'second' : 'seconds'}.`;
            ...
            // crash here line 302 map of undefined.  Not sure how this didn't
            // exist at this time.  Race condition in settimeout/interval?  Both
            // games completed at almost the same time?  Dunno.
            const otherGameWinningPlayerSocketIds =
                Object.keys(io.sockets.sockets)
                    .filter(socketId =>
                                io.sockets.sockets[socketId]
                                    .handshake.session.passport &&
                                game.general.tournyInfo
                                    .winningPlayersFirstCompletedGame
            ...
            const socketIds =
                winningPlayerSocketIds.concat(otherGameWinningPlayerSocketIds);
            ...
            finalGame.publicPlayersState =
                game.general.tournyInfo.winningPlayersFirstCompletedGame
                    .concat(game.private.seatedPlayers.filter(
                        player => player.role.team === winningTeamName))
                    .map(player => {
                      player.cardStatus = {
                        cardDisplayed : false,
            ...
            if (finalGame.general.blindMode) {
              ...
              finalGame.general.replacementNames =
                  _.shuffle(animals)
                      .slice(0, finalGame.publicPlayersState.length)
                      .map((animal, index) => `${
                               _shuffledAdjectives[index]
                                   .charAt(0)
                                   .toUpperCase()}${
            ...
            finalGame.general.name = `${
                game.general.name.slice(0, game.general.name.length -
                                               7)}-tableFINAL`;
        ...
        otherGame.general.tournyInfo.winningPlayersFirstCompletedGame =
            _.cloneDeep(game.private.seatedPlayers)
                .filter(player => player.role.team === winningTeamName);
        sendInProgressGameUpdate(game);
      }
    } else {
      ...
      game.chats.push({
        gameChat : true,
        timestamp : new Date(),
        chat : [ {text : 'The tournament has ended.'} ]
      });
