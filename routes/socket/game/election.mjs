...
import { shufflePolicies } from './common.mjs';
import { selectChancellor } from './election-util.mjs';
 ...
 * @param {object} socket - socket
 */
export const selectPresidentVoteOnVeto = (passport, game, data, socket) => {
	const { experiencedMode } = game.general;
 ...
 * @param {object} socket - socket
 */
export const selectChancellorVoteOnVeto = (passport, game, data, socket) => {
	const { experiencedMode } = game.general;
	const president = game.private.seatedPlayers[game.gameState.presidentIndex];
	const chancellorIndex = game.publicPlayersState.findIndex(player => player.governmentStatus === 'isChancellor');
 ...
 * @param {object} socket - socket
 */
export const selectChancellorPolicy = (passport, game, data, wasTimer, socket) => {
	const { experiencedMode } = game.general;
 ...
 * @param {object} socket - socket
 */
export const selectPresidentPolicy = (passport, game, data, wasTimer, socket) => {
	const { presidentIndex } = game.gameState;
	const president = game.private.seatedPlayers[presidentIndex];
	const chancellorIndex = game.publicPlayersState.findIndex(player => player.governmentStatus === 'isChancellor');
	const chancellor = game.private.seatedPlayers[chancellorIndex];
 ...
 */
export const selectVoting = (passport, game, data, socket, force = false) => {
	const { seatedPlayers } = game.private;
	const { experiencedMode } = game.general;
