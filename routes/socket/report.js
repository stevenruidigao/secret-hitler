...
function sendReport(game, report, data, type) {
  Account.find({staffRole : {$exists : true}}).then(accounts => {
    ...
    const isStaff = players.some(n => staffUserNames.includes(n) ||
...
module.exports.makeReport = (data, game, type = 'report') => {
  const {player, seat, role, election, situation, uid, gameType, homepage} =
      data;

  if (!homepage) {
    // No Auto-Reports, or Mod Pings from Custom, Unlisted, or Private Games
    if (!game || game.customGameSettings.enabled || game.general.unlistedGame ||
        game.general.private)
      return;
    // No Auto-Reports from Casual games
    if (game.general.casualGame &&
        (type === 'report' || type === 'reportdelayed'))
      return;
  }
  ...
  if (type === 'ping') {
    const httpEscapedSituation =
        situation.replace(/( |^)(https?:\/\/\S+)( |$)/gm, '$1<$2>$3');
    ...
    report = JSON.stringify({
      content : `<@&${process.env.DISCORDMODID}>\n__**Player**__: ${player} ${
          homepage
              ...
              : `\n__**Message**__: ${
                    httpEscapedSituation}\n__**Election #**__: ${
                    election}\n__**Game Type**__: ${
                    gameType}\n**<https://secrethitler.io/game/#/table/${
                    uid}>**`}`,
      username : '@Mod Ping',
      allowed_mentions : {roles : [ process.env.DISCORDMODID ]},
      avatar_url :
          'https://cdn.discordapp.com/emojis/612042360318328842.png?v=1'
    });
  ...
  if (type === 'report' || type === 'reportdelayed') {
    ...
    report = {
      ...
      allowed_mentions : {roles : [ process.env.DISCORDMODID ]},
      avatar_url :
          'https://cdn.discordapp.com/emojis/230161421336313857.png?v=1'
    };
      ...
      Account.find({username : {$in : otherPlayers}})
          .then(accounts => {
            accounts.forEach(account => {
              let ip;
              if (account)
                ip = account.lastConnectedIP || account.signupIP;

              const seat = game.private.seatedPlayers.findIndex(
                  elem => elem.userName === account.username);
              if (ip === throwerIP) {
                matches[seat] = `${account.username} {${seat + 1}}`;
              } else if (
                  ip.includes('.') && // Ensure both IPs are IPv4
                  throwerIP.includes('.') &&
                  ip.split('.').splice(0, 3).join(
                      '.') === // Splice off last block
                      throwerIP.split('.').splice(0, 3).join(
                          '.') // to determine if the IPs are a 3-block match
              ) {
                matches[seat] = `${account.username} {${seat + 1}} (3-block)`;
              } else if (
                  ip.includes(':') && // Ensure both IPs are IPv6
                  throwerIP.includes(':') &&
                  ip.split(':').splice(0, 4).join(
                      ':') === // Splice off first four blocks to check if they
                               // match
                      throwerIP.split(':').splice(0, 4).join(
                          ':') // to determine if the IPs are a 4-block match
              ) {
                matches[seat] =
                    `${account.username} {${seat + 1}} (4-block IPv6)`;
              }
            });
          })
          .then(() => {
  ...
  if (type === 'modchat' || type === 'modchatdelayed') {
    ...
    report = {
      ...
      allowed_mentions : {roles : [ process.env.DISCORDMODID ]},
      avatar_url :
          'https://cdn.discordapp.com/emojis/230161421311148043.png?v=1'
    };
    sendReport(game, report, data, type);
  }
};
