const {CURRENTSEASONNUMBER} =
    require('../../src/frontend-scripts/node-constants');
...
fs.readdirSync('public/images/emotes', {withFileTypes : true}).forEach(file => {
  if (file.name.endsWith('.png')) {
    ...
    emotes[`:${emoteName}:`] =
        `/images/emotes/${file.name}?v=${version.number}`;
  }
});
...
const getGlobalSetting =
    promisify(globalSettingsClient.get).bind(globalSettingsClient);
const setGlobalSetting =
    promisify(globalSettingsClient.set).bind(globalSettingsClient);

const globalSettingsCache =
    {}; // READ ONLY variables that are cloned from redis (they will be reset
...
module.exports.getPrivateChatTruncate =
    async () => { return globalSettingsCache['private-chat-truncate']; };
  ...
  return module.exports.userList
      .map(
          user => ({
            ...
            eloOverall : user.eloOverall ? Math.floor(user.eloOverall)
                                         : undefined,
            eloSeason : user.eloSeason ? Math.floor(user.eloSeason) : undefined,
            status :
                user.status && user.status.type && user.status.type != 'none'
                    ? user.status
                    : undefined,
            winsSeason : prune(user[`winsSeason${CURRENTSEASONNUMBER}`]),
            lossesSeason : prune(user[`lossesSeason${CURRENTSEASONNUMBER}`]),
            rainbowWinsSeason :
                prune(user[`rainbowWinsSeason${CURRENTSEASONNUMBER}`]),
            rainbowLossesSeason :
                prune(user[`rainbowLossesSeason${CURRENTSEASONNUMBER}`]),
...
module.exports.formattedGameList = () => {
  return Object.keys(module.exports.games)
      .map(gameName => ({
             name : games[gameName].general.name,
             flag : games[gameName].general.flag,
             userNames :
                 games[gameName].publicPlayersState.map(val => val.userName),
             customCardback : games[gameName].publicPlayersState.map(
                 val => val.customCardback),
             customCardbackUid : games[gameName].publicPlayersState.map(
                 val => val.customCardbackUid),
             ...
             maxPlayersCount : games[gameName].general.maxPlayersCount ||
                                   games[gameName].general.minPlayersCount,
             ...
             isVerifiedOnly :
                 games[gameName].general.isVerifiedOnly || undefined,
             ...
             flappyOnlyMode :
                 games[gameName].general.flappyOnlyMode || undefined,
             tournyStatus : (() => {
               if (games[gameName].general.isTourny) {
                 if (games[gameName].general.tournyInfo.queuedPlayers &&
                     games[gameName].general.tournyInfo.queuedPlayers.length) {
                   return {
                     queuedPlayers :
                         games[gameName].general.tournyInfo.queuedPlayers.length
                   };
             ...
             experiencedMode :
                 games[gameName].general.experiencedMode || undefined,
             playerChats : games[gameName].general.playerChats || undefined,
             disableGamechat :
                 games[gameName].general.disableGamechat || undefined,
             blindMode : games[gameName].general.blindMode || undefined,
             enactedLiberalPolicyCount :
                 games[gameName].trackState.liberalPolicyCount,
             enactedFascistPolicyCount :
                 games[gameName].trackState.fascistPolicyCount,
...
module.exports.verifyBypass = key => { return bypassKeys.indexOf(key) >= 0; };
...
BannedIP.deleteMany(
    {type : 'new', bannedDate : {$lte : unbanTime}, permanent : {$ne : true}},
    (err, r) => {
      if (err)
        throw err;
    });
    ...
    BannedIP.find({}, (err, allIPs) => {
      if (err)
        ...
        for (const potentialMatch of allIPs) {
          if (potentialMatch.ip == IP ||
              doesIPMatchCIDR(potentialMatch.ip, IP)) {
        ...
        if (ip) {
          date = Date.now();
          unbannedTime =
              ip.bannedDate.getTime() + (banLength[ip.type] || banLength.big);
        }
