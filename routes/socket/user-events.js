		...
		switch (data.claim) {
			case 'wasPresident':
				switch (data.claimState) {
					case 'rrr':
						game.private.summary = game.private.summary.updateLog({ presidentClaim: { reds: 3, blues: 0 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
					case 'rrb':
						game.private.summary = game.private.summary.updateLog({ presidentClaim: { reds: 2, blues: 1 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
					case 'rbb':
						game.private.summary = game.private.summary.updateLog({ presidentClaim: { reds: 1, blues: 2 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
					case 'bbb':
						game.private.summary = game.private.summary.updateLog({ presidentClaim: { reds: 0, blues: 3 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
				}
				if (validClaim) {
			...
			case 'wasChancellor':
				switch (data.claimState) {
					case 'rr':
						game.private.summary = game.private.summary.updateLog({ chancellorClaim: { reds: 2, blues: 0 } }, { chancellorId: playerIndex });
						validClaim = true;
						break;
					case 'rb':
						game.private.summary = game.private.summary.updateLog({ chancellorClaim: { reds: 1, blues: 1 } }, { chancellorId: playerIndex });
						validClaim = true;
						break;
					case 'bb':
						game.private.summary = game.private.summary.updateLog({ chancellorClaim: { reds: 0, blues: 2 } }, { chancellorId: playerIndex });
						validClaim = true;
						break;
				}
			...
			case 'didPolicyPeek':
				switch (data.claimState) {
					case 'rrr':
						game.private.summary = game.private.summary.updateLog({ policyPeekClaim: { reds: 3, blues: 0 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
					case 'rbr':
						game.private.summary = game.private.summary.updateLog({ policyPeekClaim: { reds: 2, blues: 1 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
					case 'brr':
						game.private.summary = game.private.summary.updateLog({ policyPeekClaim: { reds: 2, blues: 1 } }, { presidentId: playerIndex });
					...
					case 'rrb':
						...
						break;
					case 'rbb':
						game.private.summary = game.private.summary.updateLog({ policyPeekClaim: { reds: 1, blues: 2 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
					case 'bbr':
						game.private.summary = game.private.summary.updateLog({ policyPeekClaim: { reds: 1, blues: 2 } }, { presidentId: playerIndex });
					...
					case 'brb':
						...
						break;
					case 'bbb':
						game.private.summary = game.private.summary.updateLog({ policyPeekClaim: { reds: 0, blues: 3 } }, { presidentId: playerIndex });
						validClaim = true;
						break;
				}
				if (validClaim) {
...
module.exports.handleUpdatedRemakeGame = (passport, game, data, socket) => {
	...
	if (game && game.general && game.general.private) {
		chat = {
			timestamp: new Date(),
			gameChat: true,
			chat: [{ text: 'Player ' }, { text: `${passport.user} {${playerIndex + 1}} `, type: 'player' }],
		};
	} else {
		chat = {
			timestamp: new Date(),
			...
			if (game.general.playerChats === 'disabled') {
				game.private.seatedPlayers
					...
					.gameChats.push({
						...
						chat: [
							{
								text: `${publicPlayersState[affectedPlayerNumber].userName} (${affectedPlayerNumber + 1})`,
								type: 'player',
							},
		...
		if (data.isReportResolveChange) {
			...
			switch (data.action) {
				...
				case 'broadcast':
					...
					break;
				case 'ipban':
					const ipban = new BannedIP({ bannedDate: new Date(), type: 'small', ip: data.ip });
				...
				case 'fragbanLarge':
					...
					break;
				case 'timeOut':
					const timeout = new BannedIP({ bannedDate: new Date(), type: 'small', ip: data.ip });
					timeout.save(() => {
						Account.findOne({ username: data.userName })
							.then((account) => {
								if (account) {
				...
				case 'timeOut2':
					...
					break;
				case 'timeOut3':
					const timeout3 = new BannedIP({ bannedDate: new Date(), type: 'tiny', ip: data.ip });
					timeout3.save(() => {
						Account.findOne({ username: data.userName })
							.then((account) => {
								if (account) {
				...
				case 'deleteProfile':
					...
					break;
				case 'ipbanlarge':
					const ipbanl = new BannedIP({ bannedDate: new Date(), type: 'big', ip: data.ip });
