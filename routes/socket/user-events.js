...
const updateSeatedUser = (socket, passport, data) => {
  ...
  Account.findOne({username : passport.user}).then(account => {
    ...
    const isBlacklistSafe =
        !game.general.gameCreatorBlacklist ||
        !game.general.gameCreatorBlacklist.includes(passport.user);
  ...
  const newGame = {
    ...
    general : {
      ...
      name : user.isPrivate ? 'Private Game'
                            : data.gameName ? data.gameName : 'New Game',
...
module.exports.handleUpdatedRemakeGame = (passport, game, data, socket) => {
  ...
  if (game && game.general && game.general.private) {
    chat = {
      ...
      chat : [
        {text : 'Player '},
        {text : `${passport.user} {${playerIndex + 1}} `, type : 'player'}
      ]
    };
  } else {
    chat = {
    ...
    setTimeout(() => {
        ...
        if (io.sockets.sockets[id]) {
          ...
          if (io.sockets.sockets[id] && io.sockets.sockets[id].handshake &&
            ...
            if (io.sockets.sockets[id].handshake.session.passport.user ===
                newGame.general.gameCreatorName)
              creatorRemade = true;
          }
        }
      });
      if (creatorRemade && newGame.general.gameCreatorBlacklist != null) {
        const creator = userList.find(user => user.userName ===
                                              newGame.general.gameCreatorName);
        if (creator)
          newGame.general.gameCreatorBlacklist = creator.blacklist;
      } else
        newGame.general.gameCreatorBlacklist = null;
 ...
 */
module.exports.handleAddNewGameChat = (socket, passport, data, game,
                                       modUserNames, editorUserNames,
                                       adminUserNames, addNewClaim,
                                       isTourneyMod) => {
  // Authentication Assured in routes.js
  ...
  if (pingMods && player) {
    if (!game.lastModPing || Date.now() > game.lastModPing + 180000) {
      ...
      makeReport({
        ...
        uid : game.general.uid,
        gameType : game.general.casualGame
                       ? 'Casual'
                       : game.general.practiceGame ? 'Practice' : 'Ranked'
      },
                 game, 'ping');
    } else {
    ...
    if (data.isReportResolveChange) {
      ...
      case 'clearTimeoutIP':
        BannedIP.remove({ip : data.ip}, (err, res) => {
          if (err)
            socket.emit('sendAlert', `IP clear failed:\n${err}`);
        });
        break;
      ...
      case 'ipban':
        const ipban = new BannedIP(
            {bannedDate : new Date(), type : 'small', ip : data.ip});
      ...
      case 'timeOut3':
        const timeout3 = new BannedIP(
            {bannedDate : new Date(), type : 'tiny', ip : data.ip});
        timeout3.save(() => {
          Account.findOne({username : data.userName})
              .then(account => {
                if (account) {
      ...
      case 'ipbanlarge':
        const ipbanl =
            new BannedIP({bannedDate : new Date(), type : 'big', ip : data.ip});
      ...
      default:
        if (data.userName.substr(0, 7) === 'DELGAME') {
          ...
          const setType =
              /setRWins/.test(data.action.type)
                  ? 'rainbowWins'
                  : /setRLosses/.test(data.action.type)
                        ? 'rainbowLosses'
                        : /setWins/.test(data.action.type) ? 'wins' : 'losses';
          const number = setType === 'wins'
...
module.exports.handlePlayerReport = (passport, data, callback) => {
  ...
  const gameType = data.uid
                       ? (game.general.isTourny
                              ? 'tournament'
                              : game.general.casualGame ? 'casual' : 'standard')
                       : 'homepage';
