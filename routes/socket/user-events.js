...
const {
  ...
  handleAEMMessages,
  getStaffRole,
  sendInProgressModDMUpdate,
  handleDefaultIPv6Range
} = require('./util.js');
...
const displayWaitingForPlayers = game => {
  if (game.general.isTourny) {
    const count = game.general.maxPlayersCount -
                  game.general.tournyInfo.queuedPlayers.length;

    return count === 1 ? `Waiting for ${count} more player..`
                       : `Waiting for ${count} more players..`;
  }
  const includedPlayerCounts =
      _.range(game.general.minPlayersCount, game.general.maxPlayersCount + 1)
          .filter(value => !game.general.excludedPlayerCount.includes(value));
...
const startCountdown = game => {
  ...
  const countDown = setInterval(() => {
    if (game.gameState.cancellStart) {
    ...
    } else if (startGamePause === 4 || game.publicPlayersState.length ===
      ...
      if (game.general.isTourny) {
        ...
        const APlayers = players.filter(
            (player, index) => index < game.general.maxPlayersCount / 2);
        ...
        const ASocketIds =
            Object.keys(io.sockets.sockets)
                .filter(socketId => io.sockets.sockets[socketId]
                                        .handshake.session.passport &&
                                    APlayerNames.includes(
                                        io.sockets.sockets[socketId]
                                            .handshake.session.passport.user));
        const BSocketIds =
            Object.keys(io.sockets.sockets)
                .filter(socketId => io.sockets.sockets[socketId]
                                        .handshake.session.passport &&
                                    BPlayerNames.includes(
                                        io.sockets.sockets[socketId]
                                            .handshake.session.passport.user));

        gameA.general.uid = `${game.general.uid}TableA`;
        gameA.general.minPlayersCount = gameA.general.maxPlayersCount =
            game.general.maxPlayersCount / 2;
        ...
        ASocketIds.forEach(id => {
          ...
          Object.keys(socket.rooms)
              .forEach(roomUid => { socket.leave(roomUid); });
        ...
        if (game.general.blindMode) {
          ...
          gameA.general.replacementNames =
              _.shuffle(animals)
                  .slice(0, gameA.publicPlayersState.length)
                  .map((animal, index) => `${
                           _shuffledAdjectives[index].charAt(0).toUpperCase()}${
                           _shuffledAdjectives[index].slice(1)} ${animal}`);

          gameB.general.replacementNames =
              _.shuffle(animals)
        ...
        game.remakeData =
            game.publicPlayersState.map(player => ({
      ...
      game.general.status =
          game.general.isTourny
              ? `Tournament starts in ${startGamePause} second${
                    startGamePause === 1 ? '' : 's'}.`
              : `Game starts in ${startGamePause} second${
                    startGamePause === 1 ? '' : 's'}.`;
...
const checkStartConditions = game => {
  ...
  if (game.gameState.isStarted &&
      (game.publicPlayersState.length < game.general.minPlayersCount ||
       game.general.excludedPlayerCount.includes(
...
const playerLeavePretourny = (game, playerName) => {
  ...
  queuedPlayers.splice(
      queuedPlayers.findIndex(player => player.userName === playerName), 1);
  ...
  game.chats.push({
    ...
    chat : [
      {text : playerName, type : 'player'}, {
        text : ` (${queuedPlayers.length}/${
            game.general.maxPlayersCount}) has left the tournament queue.`
      }
    ]
  });
  game.general.status = displayWaitingForPlayers(game);
...
const handleSocketDisconnect = socket => {
  ...
  if (passport && Object.keys(passport).length) {
    const userIndex =
        userList.findIndex(user => user.userName === passport.user);
    const gameNamesPlayerSeatedIn = Object.keys(games).filter(
        gameName => games[gameName].publicPlayersState.find(
            player => player.userName === passport.user && !player.leftGame));
    ...
    if (gameNamesPlayerSeatedIn.length) {
      gameNamesPlayerSeatedIn.forEach(gameName => {
        ...
        const playerIndex = publicPlayersState.findIndex(
            player => player.userName === passport.user);

        if ((!gameState.isStarted && publicPlayersState.length === 1) ||
            (gameState.isCompleted &&
             publicPlayersState
                     .filter(player => !player.connected || player.leftGame)
                     .length === game.general.playerCount - 1)) {
          ...
          const playerRemakeData =
              game.remakeData &&
              game.remakeData.find(player => player.userName === passport.user);
          if (playerRemakeData && playerRemakeData.isRemaking) {
            ...
            const remakePlayerCount =
                game.remakeData.filter(player => player.isRemaking).length;

            if (!game.general.isRemade && game.general.isRemaking &&
            ...
            chat.chat.push({
              text : ` has left and rescinded their vote to ${
                  game.general.isTourny ? 'cancel this tournament.'
                                        : 'remake this game.'} (${
          ...
          sendInProgressGameUpdate(game);
          if (game.publicPlayersState
                  .filter(publicPlayer => publicPlayer.leftGame)
                  .length === game.general.playerCount) {
    ...
    // 			game.general.tournyInfo.queuedPlayers.map(player =>
    // player.userName).includes(passport.user)
    // 	);
...
const handleUserLeaveGame = (socket, game, data, passport) => {
  ...
  const playerIndex = game.publicPlayersState.findIndex(
      player => player.userName === passport.user);
  ...
  if (playerIndex > -1) {
    ...
    if (playerRemakeData && playerRemakeData.isRemaking) {
      ...
      const minimumRemakeVoteCount =
          (game.customGameSettings.fascistCount &&
           game.general.playerCount - game.customGameSettings.fascistCount) ||
    ...
    if (game.publicPlayersState.filter(publicPlayer => publicPlayer.leftGame)
            .length === game.general.playerCount) {
      game.general.timeAbandoned = new Date();
    }
    if (!game.gameState.isTracksFlipped) {
      game.publicPlayersState.splice(
          game.publicPlayersState.findIndex(player => player.userName ===
                                                      passport.user),
          1);
      checkStartConditions(game);
  ...
  if (game.general.isTourny && game.general.tournyInfo.round === 0 &&
      passport &&
      game.general.tournyInfo.queuedPlayers.map(player => player.userName)
          .find(name => name === passport.user)) {
    playerLeavePretourny(game, passport.user);
  }

  if ((!game.publicPlayersState.length &&
       !(game.general.isTourny && game.general.tournyInfo.round === 0)) ||
      (game.general.isTourny && game.general.tournyInfo.round === 0 &&
       !game.general.tournyInfo.queuedPlayers.length)) {
...
module.exports.handleUpdatedPlayerNote = (socket, data) => {
  PlayerNote.findOne({userName : data.userName, notedUser : data.notedUser})
      .then(note => {
        if (note) {
          note.note = data.note;
          note.save(() => {
            sendPlayerNotes(
                socket,
                {userName : data.userName, seatedPlayers : [ data.notedUser ]});
          });
        } else {
...
module.exports.handleUpdatedTheme = (socket, passport, data) => {
  const fields = [
    'primaryColor', 'secondaryColor', 'tertiaryColor', 'backgroundColor',
    'textColor'
  ];
...
const updateSeatedUser = (socket, passport, data) => {
  ...
  const isBlacklistSafe =
      !game.private.gameCreatorBlacklist ||
      !game.private.gameCreatorBlacklist.includes(
          passport.user); // we can check blacklist before hitting mongo
  ...
  Account.findOne({username : passport.user}).then(account => {
    ...
    if (isNotMaxedOut && isNotInGame && isRainbowSafe && isPrivateSafe &&
      ...
      if (game.general.isTourny) {
        if (game.general.tournyInfo.queuedPlayers.map(player => player.userName)
        ...
        game.chats.push({
          ...
          gameChat : true,
          chat : [
            {text : `${passport.user}`, type : 'player'}, {
              text : ` (${game.general.tournyInfo.queuedPlayers.length}/${
                  game.general
                      .maxPlayersCount}) has entered the tournament queue.`
            }
          ]
        });
      } else {
...
module.exports.handleAddNewGame = (socket, passport, data) => {
  ...
  let a;
  let playerCounts = [];
  for (a = Math.max(data.minPlayersCount, 5);
       a <= Math.min(10, data.maxPlayersCount); a++) {
  ...
  if (!data.gameName || data.gameName.length > 20 ||
      !LEGALCHARACTERS(data.gameName)) {
    // Should be enforced on the client. Copy-pasting characters can get past
    // the LEGALCHARACTERS client check.
    return;
  }

  if (data.eloSliderValue && (user.eloSeason < data.eloSliderValue ||
                              user.eloOverall < data.eloSliderValue)) {
    return;
  }

  if (data.customGameSettings && data.customGameSettings.enabled) {
    if (!data.customGameSettings.deckState ||
        !data.customGameSettings.trackState)
      return;

    const validPowers = [
      'investigate', 'deckpeek', 'election', 'bullet', 'reverseinv', 'peekdrop'
    ];
    if (!data.customGameSettings.powers ||
        data.customGameSettings.powers.length != 5)
      return;
    for (let a = 0; a < 5; a++) {
      if (data.customGameSettings.powers[a] == '' ||
          data.customGameSettings.powers[a] == 'null')
        data.customGameSettings.powers[a] = null;
      else if (data.customGameSettings.powers[a] &&
               !validPowers.includes(data.customGameSettings.powers[a]))
        return;
    }

    if (!(data.customGameSettings.hitlerZone >= 1) ||
        data.customGameSettings.hitlerZone > 5)
      return;
    if (!data.customGameSettings.vetoZone ||
    ...
    // Custom games should probably require a fixed player count, which will be
    // in playerCounts[0] regardless.
    if (!(data.customGameSettings.fascistCount >= 1) ||
        data.customGameSettings.fascistCount + 1 > playerCounts[0] / 2)
      return;

    // Ensure standard victory conditions can be met for both teams.
    if (!(data.customGameSettings.deckState.lib >= 5) ||
        data.customGameSettings.deckState.lib > 8)
      return;
    if (!(data.customGameSettings.deckState.fas >= 6) ||
        data.customGameSettings.deckState.fas > 19)
      return;

    // Roundabout way of checking for null/undefined but not 0.
    if (!(data.customGameSettings.trackState.lib >= 0) ||
        data.customGameSettings.trackState.lib > 4)
      return;
    if (!(data.customGameSettings.trackState.fas >= 0) ||
        data.customGameSettings.trackState.fas > 5)
      return;

    // Need at least 13 cards (11 on track plus two left-overs) to ensure that
    // the deck does not run out.
    if (data.customGameSettings.deckState.lib +
    ...
    playerCounts = [
      playerCounts[0]
    ]; // Lock the game to a specific player count. Eventually there should be
       // one set of settings per size.
  } else {
    data.customGameSettings = {enabled : false};
  }
  ...
  const newGame = {
    ...
    general : {
      whitelistedPlayers : [],
      uid : data.isTourny ? `${generateCombination(3, '', true)}Tournament`
                          : uid,
      name : user.isPrivate  ? 'Private Game'
             : data.gameName ? data.gameName
                             : 'New Game',
      flag : data.flag || 'none', // TODO: verify that the flag exists, or that
                                  // an invalid flag does not cause issues
      ...
      playerChats : data.playerChats === 'emotes' &&
                            [ 'casual', 'practice' ].includes(data.gameType)
                        ? 'emotes'
                    : data.playerChats === 'emotes' ? 'enabled'
                                                    : data.playerChats,
      isVerifiedOnly : data.isVerifiedOnly,
      disableObserverLobby : data.disableObserverLobby,
      disableObserver :
          data.disableObserverLobby || (data.disableObserver && !data.isTourny),
      isTourny : false,
      lastModPing : 0,
      ...
      timedMode : typeof data.timedMode === 'number' && data.timedMode >= 2 &&
                          data.timedMode <= 6000
                      ? data.timedMode
                      : false,
      flappyMode : data.flappyMode,
      flappyOnlyMode : data.flappyMode && data.flappyOnlyMode,
      casualGame : data.casualGame || (typeof data.timedMode === 'number' &&
                                       data.timedMode < 30)
                       ? true
      ...
      private : user.isPrivate
                    ? (data.privatePassword ? data.privatePassword : 'private')
  ...
  if (newGame.customGameSettings.enabled) {
    let chat = {
      ...
      chat : [
        {text : 'There will be '}, {
          text : `${
              newGame.customGameSettings.deckState.lib -
              newGame.customGameSettings.trackState.lib} liberal`,
          type : 'liberal'
        },
        {text : ' and '}, {
          text : `${
              newGame.customGameSettings.deckState.fas -
              newGame.customGameSettings.trackState.fas} fascist`,
          type : 'fascist'
        },
  ...
  if (data.isTourny) {
    ...
    newGame.general.minPlayersCount = newGame.general.maxPlayersCount =
        minPlayersCount === 1   ? 14
        : minPlayersCount === 2 ? 16
                                : 18;
    newGame.general.status =
        `Waiting for ${newGame.general.minPlayersCount - 1} more players..`;
    newGame.chats.push({
      timestamp : new Date(),
      gameChat : true,
      chat : [
        {text : `${user.userName}`, type : 'player'}, {
          text : ` (${data.general.tournyInfo.queuedPlayers.length}/${
              data.general.maxPlayersCount}) has entered the tournament queue.`
        }
      ]
    });
  }
...
module.exports.handleAddNewClaim = (socket, passport, game, data) => {
  ...
  if (game && game.private && game.private.seatedPlayers &&
      ...
      !/^(wasPresident|wasChancellor|didSinglePolicyPeek|didPolicyPeek|didInvestigateLoyalty)$/
           .exec(game.private.seatedPlayers[playerIndex]
                     .playersState[playerIndex]
                     .claim)) {
    return;
  }
  ...
  const chat = (() => {
    ...
    case 'wasPresident':
      ...
      if (validClaim) {
        text = [
          ...
          {text : 'claims '}, {claim : data.claimState}, {text : '.'}
        ];
        return text;
      }
      return;
    ...
    case 'didSinglePolicyPeek':
      if (data.claimState === 'liberal' || data.claimState === 'fascist') {
        text = [
          ...
          {text : data.claimState, type : data.claimState}, {text : ' policy.'}
        ];
        return text;
      }
    case 'didPolicyPeek':
      ...
      if (validClaim) {
        text = [
          ...
          {text : 'claims to have peeked at '}, {claim : data.claimState},
          {text : '.'}
        ];
        return text;
      }
    ...
    case 'didInvestigateLoyalty':
        ...
        text = [
          ...
          {text : 'sees the party membership of '}, {
            text : blindMode
                       ...
                       : `${
                             game.private.seatedPlayers[invIndex] &&
                             game.private.seatedPlayers[invIndex].userName} {${
                             invIndex + 1}} `,
            type : 'player'
          },
      ...
      game.private.summary =
          game.private.summary.updateLog({investigationClaim : data.claimState},
                                         {investigatorId : playerIndex});

      switch (data.claimState) {
      case 'fascist':
        text.push({text : 'fascist ', type : 'fascist'}, {text : 'team.'});

        return text;
      case 'liberal':
        text.push({text : 'liberal ', type : 'liberal'}, {text : 'team.'});
        return text;
      }
    }
  })();
...
module.exports.handleUpdatedRemakeGame = (passport, game, data, socket) => {
  ...
  if (game.gameState.isGameFrozen) {
    if (socket) {
      socket.emit(
          'sendAlert',
          'An AEM member has prevented this game from proceeding. Please wait.');
    }
    return;
  }
  ...
  if (!remakeData)
    return;
  const playerIndex =
      remakeData.findIndex(player => player.userName === passport.user);
  const realPlayerIndex =
      publicPlayersState.findIndex(player => player.userName === passport.user);
  ...
  if (game && game.general && game.general.private &&
      !game.general.privateAnonymousRemakes) {
    chat = {
      timestamp : new Date(),
      gameChat : true,
      chat : [
        {text : 'Player '},
        {text : `${passport.user} {${realPlayerIndex + 1}} `, type : 'player'}
      ]
    };
  } else {
    chat = {
  ...
  const makeNewGame = () => {
    ...
    const remakePlayerNames = remakeData.filter(player => player.isRemaking)
                                  .map(player => player.userName);
    const remakePlayerSocketIDs =
        Object.keys(io.sockets.sockets)
            .filter(
                socketId =>
                    io.sockets.sockets[socketId].handshake.session.passport &&
                    remakePlayerNames.includes(
                        io.sockets.sockets[socketId]
                            .handshake.session.passport.user));
    sendInProgressGameUpdate(game);
    ...
    if (game.general.uid.indexOf('Remake') === -1) {
      ...
      newGame.general.uid = `${game.general.uid.split('Remake')[0]}Remake${
          parseInt(game.general.uid.split('Remake')[1]) + 1}`;
    }
    ...
    newGame.publicPlayersState =
        game.publicPlayersState
            .filter(
                player =>
                    game.remakeData.filter(rmkPlayer => rmkPlayer.isRemaking)
    ...
    game.publicPlayersState.forEach((player, i) => {
      if (game.private.seatedPlayers && game.private.seatedPlayers[i] &&
          game.private.seatedPlayers[i].role) {
    ...
    setTimeout(() => {
      game.publicPlayersState.forEach(player => {
        if (remakePlayerNames.includes(player.userName))
      ...
      remakePlayerSocketIDs.forEach((id, index) => {
        if (io.sockets.sockets[id]) {
          ...
          if (io.sockets.sockets[id] && io.sockets.sockets[id].handshake &&
              io.sockets.sockets[id].handshake.session &&
              io.sockets.sockets[id].handshake.session.passport) {
            updateSeatedUser(io.sockets.sockets[id],
                             io.sockets.sockets[id].handshake.session.passport,
                             {uid : newGame.general.uid});
            // handleUserLeaveGame(io.sockets.sockets[id], passport,
            // game, {isSeated: true, isRemake: true});
            if (io.sockets.sockets[id].handshake.session.passport.user ===
                newGame.private.gameCreatorName)
              creatorRemade = true;
          }
        }
      });
      if (creatorRemade && newGame.private.gameCreatorBlacklist != null) {
        const creator = userList.find(user => user.userName ===
                                              newGame.private.gameCreatorName);
        if (creator)
          newGame.private.gameCreatorBlacklist = creator.blacklist;
      } else
  ...
  const cancellTourny = firstTableUid => {
    ...
    if (secondTable) {
      ...
      secondTable.chats.push({
        ...
        chat : [ {
          text :
              'Due to the other tournament table voting for cancellation, this tournament has been cancelled.',
          type : 'hitler'
        } ]
      });
  ...
  if (data.remakeStatus && Date.now() > player.remakeTime + 7000) {
    ...
    const remakePlayerCount =
        remakeData.filter(player => player.isRemaking).length;
    chat.chat.push({
      text : ` has voted to ${remakeText} this ${
          game.general.isTourny
              ? 'tournament.'
              : 'game.'} (${remakePlayerCount}/${minimumRemakeVoteCount})`
    });

    if (!game.general.isRemaking && publicPlayersState.length > 3 &&
        remakePlayerCount >= minimumRemakeVoteCount) {
      ...
      game.private.remakeTimer = setInterval(() => {
        if (game.general.remakeCount !== 0) {
          game.general.status =
              `Game is ${game.general.isTourny ? 'cancelled ' : 'remade'} in ${
                  game.general.remakeCount} ${
                  game.general.remakeCount === 1 ? 'second' : 'seconds'}.`;
          game.general.remakeCount--;
        } else {
          clearInterval(game.private.remakeTimer);
          game.general.status = `Game has been ${
              game.general.isTourny ? 'cancelled' : 'remade'}.`;
          ...
          const remainingPoliciesChat = {
            ...
            chat : [
              {text : 'The remaining policies are '}, {
                policies : game.private.policies.map(
                    policyName => (policyName === 'liberal' ? 'b' : 'r'))
              },
              {text : '.'}
            ]
          };
          ...
          game.private.seatedPlayers.forEach(
              player => { player.gameChats.push(remainingPoliciesChat); });
    ...
    chat.chat.push({
      text : ` has rescinded their vote to ${
          game.general.isTourny ? 'cancel this tournament.'
                                : 'remake this game.'} (${remakePlayerCount}/${
 ...
 */
module.exports.handleAddNewGameChat =
    async (socket, passport, data, game, modUserNames, editorUserNames,
           adminUserNames, addNewClaim, isTourneyMod) => {
  // Authentication Assured in routes.js
  if (!game || !game.general || !data.chat)
    return;
  const chat = data.chat.trim();
  const staffUserNames =
      [...modUserNames, ...editorUserNames, ...adminUserNames ];
  const playerIndex = game.publicPlayersState.findIndex(
      player => player.userName === passport.user);

  if (chat.length > 300 || !chat.length ||
      /^(\*|(\*|~|_){2,4})$/i.exec(data.chat)) {
    return;
  }

  const {publicPlayersState} = game;
  const player =
      publicPlayersState.find(player => player.userName === passport.user);
  ...
  const AEM = staffUserNames.includes(passport.user) ||
  ...
  if (!((AEM || (isTourneyMod && game.general.unlistedGame)) &&
    ...
    if ((!game.gameState.isStarted || game.gameState.isCompleted) &&
        game.general.disableObserverLobby && playerIndex === -1) {
      return;
    }
  }
  ...
  if (game && game.private && game.private.seatedPlayers &&
    ...
    if (/^[RB]{2,3}$/i.exec(chat)) {
      const formattedChat =
          chat.toLowerCase().split('').sort().reverse().join('');

      // console.log(chat, ' - ', formattedChat, ' - ',
      // game.private.seatedPlayers[playerIndex].playersState[playerIndex].claim);

      if (chat.length === 3 && 0 <= playerIndex <= 9 &&
          game.private.seatedPlayers[playerIndex]
                  .playersState[playerIndex]
                  .claim === 'wasPresident') {
        const claimData = {
          ...
          claim : game.private.seatedPlayers[playerIndex]
                      .playersState[playerIndex]
                      .claim,
          uid : data.uid
        };
      ...
      if (chat.length === 2 && 0 <= playerIndex <= 9 &&
          game.private.seatedPlayers[playerIndex]
                  .playersState[playerIndex]
                  .claim === 'wasChancellor') {
        const claimData = {
    ...
    if (/^(b|blue|l|lib|liberal)$/i.exec(chat)) {
      // console.log(chat, ' - ', 'liberal', ' - ',
      // game.private.seatedPlayers[playerIndex].playersState[playerIndex].claim);
      if (0 <= playerIndex <= 9 &&
          (game.private.seatedPlayers[playerIndex]
                   .playersState[playerIndex]
                   .claim === 'didSinglePolicyPeek' ||
           game.private.seatedPlayers[playerIndex]
                   .playersState[playerIndex]
                   .claim === 'didInvestigateLoyalty')) {
        const claimData = {
          userName : user.userName,
          claimState : 'liberal',
          claim : game.private.seatedPlayers[playerIndex]
  ...
  if (player &&
      ((player.isDead && !game.gameState.isCompleted) || player.leftGame)) {
    return;
  }
  ...
  if (!(AEM || (isTourneyMod && game.general.unlistedGame))) {
    if (!player) {
      ...
      if ((!game.gameState.isStarted || game.gameState.isCompleted) &&
          game.general.disableObserverLobby) {
        return;
      }
    }
  }
  ...
  if (player &&
      (gameState.phase === 'presidentSelectingPolicy' ||
       gameState.phase === 'chancellorSelectingPolicy') &&
      (publicPlayersState.find(play => play.userName === player.userName)
               .governmentStatus === 'isPresident' ||
       publicPlayersState.find(play => play.userName === player.userName)
               .governmentStatus === 'isChancellor')) {
    return;
  }
  ...
  if (AEM) {
    ...
    if (aemRigdeck) {
      if (game && game.private) {
        ...
        if (/^([RB]{1,27})$/i.exec(deck)) {
          ...
          changedChat.push({text : '.'});

          game.chats.push(
              {gameChat : true, timestamp : new Date(), chat : changedChat});
    ...
    if (aemRigrole) {
      if (game && game.private) {
        ...
        if (/^(hitler|fascist|liberal)$/i.exec(role) &&
            parseInt(player, 10) < publicPlayersState.length + 1 &&
            parseInt(player, 10) > 0) {
          const changedChat =
          ...
          changedChat.push({text : ' to '});

          changedChat.push({text : role, type : role});
    ...
    const aemForce =
        /^\/forcevote (\d{1,2}) (ya|ja|nein|yes|no|true|false)$/i.exec(chat);
    if (aemForce) {
      if (player) {
      ...
      if (game && game.private && game.private.seatedPlayers) {
        ...
        if (!affectedPlayer) {
          socket.emit('sendAlert',
                      `There is no seat {${affectedPlayerNumber + 1}}.`);
          return;
        }
        let vote = false;
        if (voteString == 'ya' || voteString == 'ja' || voteString == 'yes' ||
            voteString == 'true')
          vote = true;

        if (affectedPlayer.voteStatus.hasVoted) {
          socket.emit('sendAlert',
                      `${affectedPlayer.userName} {${
                          affectedPlayerNumber +
                          1}} has already voted.\nThey were voting: ${
                          affectedPlayer.voteStatus.didVoteYes
                              ? 'ja'
                              : 'nein'}\nYou have set them to vote: ${
                          vote ? 'ja' : 'nein'}
						`);
        }
        ...
        game.chats.push({
          ...
          chat : [
            {text : 'An AEM member has forced '}, {
              text : blindMode ? `${replacementNames[affectedPlayerNumber]} {${
        ...
        const modOnlyChat = {
          ...
          chat : [
            ...
            {text : ' to vote '},
            {text : `${vote ? 'ja' : 'nein'}`, type : 'player'}, {text : ', '},
            {text : `${affectedPlayer.userName}`, type : 'player'}, {
              text : `${
                  affectedPlayer.voteStatus.hasVoted ? ' had originally voted '
                                                     : ' had not voted.'}`
            },
            {
              text : `${
                  affectedPlayer.voteStatus.hasVoted
        ...
        selectVoting({user : affectedPlayer.userName}, game, {vote}, null,
                     true);
        sendPlayerChatUpdate(game, data);
        sendInProgressGameUpdate(game, false);
      } else {
    ...
    if (aemSkip) {
      if (player) {
        socket.emit('sendAlert',
                    'You cannot force skip a government whilst playing.');
        return;
      }
      if (game.general.isRemade) {
        socket.emit('sendAlert', 'This game has been remade.');
      ...
      if (game && game.private && game.private.seatedPlayers) {
        ...
        if (affectedPlayerNumber !== game.gameState.presidentIndex) {
          socket.emit('sendAlert',
                      `The player in seat ${
                          affectedPlayerNumber + 1} is not president.`);
          return;
        }
        let chancellor = -1;
        ...
        for (let i = 0; i < game.private.seatedPlayers.length; i++) {
          currentPlayers[i] =
              !(game.private.seatedPlayers[i].isDead ||
                (i === game.gameState.previousElectedGovernment[0] &&
        ...
        selectChancellor(null, {user : affectedPlayer.userName}, game,
                         {chancellorIndex : chancellor}, true);
        setTimeout(() => {
          for (const p of game.private.seatedPlayers.filter(
    ...
    if (aemPick) {
      ...
      if (game && game.private && game.private.seatedPlayers) {
        const affectedPlayer = game.private.seatedPlayers[affectedPlayerNumber];
        const affectedChancellor =
            game.private.seatedPlayers[chancellorPick - 1];
        if (!affectedPlayer) {
          socket.emit('sendAlert',
                      `There is no seat ${affectedPlayerNumber + 1}.`);
          return;
        ...
        if (game.publicPlayersState[chancellorPick - 1].isDead ||
            chancellorPick - 1 === affectedPlayerNumber ||
            chancellorPick - 1 ===
                game.gameState.previousElectedGovernment[1] ||
            (chancellorPick - 1 ===
                 game.gameState.previousElectedGovernment[0] &&
             game.general.livingPlayerCount > 5)) {
          socket.emit(
              'sendAlert',
              `The player in seat ${
                  chancellorPick} is not a valid chancellor. (Dead or TL)`);
          return;
        }
        ...
        game.chats.push({
          ...
          chat : [
            ...
            {text : ' to pick '}, {
              text : blindMode
                         ? `${replacementNames[chancellorPick - 1]} {${
                               chancellorPick}} `
        ...
        selectChancellor(null, {user : affectedPlayer.userName}, game,
                         {chancellorIndex : chancellorPick - 1}, true);
        sendPlayerChatUpdate(game, data);
        sendInProgressGameUpdate(game, false);
      } else {
        socket.emit('sendAlert', 'The game has not started yet.');
      ...
      if (game && game.private && game.private.seatedPlayers) {
        ...
        try {
          const affectedSocketId =
              Object.keys(io.sockets.sockets)
                  .find(socketId =>
                            ...
                            io.sockets.sockets[socketId]
                                    .handshake.session.passport.user ===
                                game.publicPlayersState[affectedPlayerNumber]
                                    .userName);
          ...
          io.sockets.sockets[affectedSocketId].emit(
              'pingPlayer', 'Secret Hitler IO: A moderator has pinged you.');
        } catch (e) {
  ...
  if (pingMods && player) {
    if (!game.lastModPing || Date.now() > game.lastModPing + 180000) {
      Account
          .find({
            username :
                {$in : game.publicPlayersState.map(player => player.userName)}
          })
          .then(accounts => {
            const staffInGame =
                accounts
                    .filter(account => account.staffRole === 'altmod' ||
            ...
            if (staffInGame.length !== 0) {
              ...
              makeReport({
                ...
                uid : game.general.uid,
                gameType : game.general.casualGame     ? 'Casual'
                           : game.general.practiceGame ? 'Practice'
                                                       : 'Ranked'
              },
                         game, 'ping');
            }
      ...
      socket.emit('sendAlert', `You can't ping mods for another ${
                                   (game.lastModPing + 180000 - Date.now()) /
                                   1000} seconds.`);
    }
    return;
  }
  ...
  for (repl of chatReplacements) {
    ...
    if (replace) {
      if (AEM) {
        if (game.general.chatReplTime[repl.id] === 0 ||
            Date.now() >
                game.general.chatReplTime[repl.id] + repl.aemCooldown * 1000) {
          data.chat = repl.replacement;
          game.general.chatReplTime[repl.id] = game.general.chatReplTime[0] =
              Date.now();
        } else {
          socket.emit('sendAlert', `You can do this command again in ${
                                       ((game.general.chatReplTime[repl.id] +
                                         repl.aemCooldown * 1000 - Date.now()) /
                                        1000)
                                           .toFixed(2)} seconds.`);
          return;
        }
      } else if (user.wins + user.losses > repl.normalGames) {
        if (Date.now() > game.general.chatReplTime[0] + 30000 &&
            (game.general.chatReplTime[repl.id] === 0 ||
             Date.now() > game.general.chatReplTime[repl.id] +
                              repl.normalCooldown * 1000)) {
          data.chat = repl.replacement;
          game.general.chatReplTime[repl.id] = game.general.chatReplTime[0] =
              Date.now();
        } else {
          socket.emit(
              'sendAlert',
              `You can't do this right now, try again in ${
                  Math.max((game.general.chatReplTime[0] + 30000 - Date.now()) /
                               1000,
                           (game.general.chatReplTime[repl.id] +
                            repl.normalCooldown * 1000 - Date.now()) /
                               1000)
                      .toFixed(2)} seconds.`);
          return;
        }
  ...
  if (pinged && player && game.gameState.isStarted &&
    ...
    try {
      ...
      if (game.general.playerChats === 'disabled') {
        game.private.seatedPlayers.find(x => x.userName === player.userName)
            .gameChats.push({
              timestamp : new Date(),
              gameChat : true,
              chat : [
                {
                  text : `${
                      game.general.blindMode
                          ? ''
                          : publicPlayersState[affectedPlayerNumber]
                                .userName} {${affectedPlayerNumber + 1}}`,
                  type : 'player'
                },
        ...
        game.private.hiddenInfoChat.push({
          ...
          gameChat : true,
          chat : [ {
            text : `${player.userName} has pinged ${
        ...
        game.chats.push({
          ...
          chat : [ {
            text : game.general.blindMode
                       ? `A player has pinged player number ${
                             affectedPlayerNumber + 1}.`
    ...
    const lastMessage =
        game.chats
            .filter(chat => !chat.gameChat && typeof chat.chat === 'string' &&
                            chat.userName === user.userName)
            .reduce(
                (acc,
                 cur) => { return acc.timestamp > cur.timestamp ? acc : cur; },
                {timestamp : new Date(0)});

    if (lastMessage.chat) {
      let leniency; // How much time (in seconds) must pass before allowing the
                    // message.
      ...
      if (!AEM && timeSince < leniency * 1000)
        return; // Prior chat was too recent.
    }

    data.staffRole = (() => {
      if (modUserNames.includes(passport.user) ||
          newStaff.modUserNames.includes(passport.user)) {
        return 'moderator';
      } else if (editorUserNames.includes(passport.user) ||
                 newStaff.editorUserNames.includes(passport.user)) {
        return 'editor';
    ...
    const privateChatTruncate =
        await getPrivateChatTruncate(); // positive integer to represent the
                                        // chats to truncate at or any falsy
                                        // value to disable
    if (privateChatTruncate && game.general.private &&
        game.chats.length >= privateChatTruncate) {
      game.chats = game.chats.slice(game.chats.length - privateChatTruncate,
                                    game.chats.length);
    }
    ...
    if (!game.gameState.isCompleted && game.gameState.isStarted) {
      if (game.general.playerChats === 'emotes' &&
        ...
        if (!newChat.length)
          return;
        data.chat = newChat;
      } else if (game.general.playerChats === 'disabled' &&
                 !game.gameState.isCompleted && game.gameState.isStarted &&
                 playerIndex !== -1) {
        // silent games
 ...
 * @param {array} adminUserNames - list of admins
 */
module.exports.handleNewGeneralChat = async (
    socket, passport, data, modUserNames, editorUserNames, adminUserNames) => {
  ...
  const AEM = user.staffRole && user.staffRole !== 'altmod' &&
              user.staffRole !== 'trialmod' && user.staffRole !== 'veteran';

  const curTime = new Date();
  const lastMessage =
      generalChats.list.filter(chat => chat.userName === user.userName)
          .reduce((acc, cur) => { return acc.time > cur.time ? acc : cur; },
                  {time : new Date(0)});
  ...
  if (pingMods) {
    try {
      ...
      if (!lastModPing || Date.now() > lastModPing + 180000) {
        ...
        socket.emit('sendAlert',
                    `You can't ping mods for another ${
                        (lastModPing + 180000 - Date.now()) / 1000} seconds.`);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  for (repl of chatReplacements) {
    ...
    if (replace) {
      if (AEM) {
        if (generalChatReplTime[repl.id] === 0 ||
            Date.now() >
                generalChatReplTime[repl.id] + repl.aemCooldown * 1000) {
          data.chat = repl.replacement;
          generalChatReplTime[repl.id] = generalChatReplTime[0] = Date.now();
        } else {
          socket.emit('sendAlert', `You can do this command again in ${
                                       ((generalChatReplTime[repl.id] +
                                         repl.aemCooldown * 1000 - Date.now()) /
                                        1000)
                                           .toFixed(2)} seconds.`);
          return;
        ...
        if (Date.now() > generalChatReplTime[0] + 30000 &&
            (generalChatReplTime[repl.id] === 0 ||
             Date.now() >
                 generalChatReplTime[repl.id] + repl.normalCooldown * 1000)) {
          data.chat = repl.replacement;
          generalChatReplTime[repl.id] = generalChatReplTime[0] = Date.now();
        } else {
          socket.emit(
              'sendAlert',
              `You can't do this right now, try again in ${
                  Math.max((generalChatReplTime[0] + 30000 - Date.now()) / 1000,
                           (generalChatReplTime[repl.id] +
                            repl.normalCooldown * 1000 - Date.now()) /
                               1000)
                      .toFixed(2)} seconds.`);
...
module.exports.handleUpdatedGameSettings = (socket, passport, data) => {
  ...
  Account.findOne({username : passport.user})
      .then(account => {
        ...
        const userIdx =
            userList.findIndex(user => user.userName === passport.user);
        const aem = account.staffRole && (account.staffRole === 'moderator' ||
                                          account.staffRole === 'editor' ||
                                          account.staffRole === 'admin');
        ...
        for (const setting in data) {
          ...
          const restrictedSettings = [
            'blacklist', 'staffDisableVisibleElo', 'staffDisableStaffColor',
            'staffIncognito', 'newReport', 'previousSeasonAward',
            'specialTournamentStatus', 'ignoreIPBans', 'tournyWins'
          ];

          if (!restrictedSettings.includes(setting) ||
          ...
          if (setting === 'staffIncognito' && aem) {
            const userListInfo = {
              ...
              staffDisableVisibleElo :
                  account.gameSettings.staffDisableVisibleElo,
              staffDisableStaffColor :
                  account.gameSettings.staffDisableStaffColor,
              ...
              specialTournamentStatus :
                  account.gameSettings.specialTournamentStatus,
              eloOverall : account.eloOverall,
              eloSeason : account.eloSeason,
              status : {type : 'none', gameId : null}
            };

            userListInfo[`winsSeason${currentSeasonNumber}`] =
                account[`winsSeason${currentSeasonNumber}`];
            userListInfo[`lossesSeason${currentSeasonNumber}`] =
                account[`lossesSeason${currentSeasonNumber}`];
            userListInfo[`rainbowWinsSeason${currentSeasonNumber}`] =
                account[`rainbowWinsSeason${currentSeasonNumber}`];
        ...
        if (((data.isPrivate && !currentPrivate) ||
             (!data.isPrivate && currentPrivate)) &&
            (!account.gameSettings.privateToggleTime ||
             account.gameSettings.privateToggleTime < Date.now() - 64800000)) {
          account.gameSettings.privateToggleTime = Date.now();
          account.save(() => { socket.emit('manualDisconnection'); });
        } else {
...
module.exports.handleSubscribeModChat = (socket, passport, game) => {
  ...
  if (game.private.hiddenInfoShouldNotify) {
    makeReport({
      player : passport.user,
      situation :
          `has subscribed to mod chat for a game without an auto-report.`,
      election : game.general.electionCount,
      title : game.general.name,
      uid : game.general.uid,
      gameType : game.general.casualGame     ? 'Casual'
  ...
  const modOnlyChat = {
    ...
    gameChat : true,
    chat : [
      {text : `${passport.user} has subscribed to mod chat. Current deck: `}
    ]
  };
  game.private.policies.forEach(policy => {
    modOnlyChat.chat.push(
        {text : policy === 'liberal' ? 'B' : 'R', type : policy});
  });
  game.private.hiddenInfoChat.push(modOnlyChat);
...
module.exports.handleGameFreeze = (socket, passport, game, modUserName) => {
  ...
  if (gameToFreeze && gameToFreeze.private &&
      gameToFreeze.private.seatedPlayers) {
  ...
  if (!game.private.gameFrozen) {
    ...
    ModAction
        .findOne({userActedOn : game.general.uid, actionTaken : 'Game Freeze'})
        .then(action => {
          if (action.modNotes) {
        ...
        .catch(err => { console.log(err, 'err in finding player report'); });
  }
  ...
  if (game.gameState.isGameFrozen) {
    if (now - game.gameState.isGameFrozen >= 4000) {
      ...
      // socket.emit('sendAlert', `You cannot do this yet, please wait
      // ${Math.ceil((now - game.gameState.isGameFrozen) / 1000)} seconds`);
      return;
    }
  } else {
  ...
  gameToFreeze.chats.push({
    ...
    chat :
        `has ${game.gameState.isGameFrozen ? 'frozen' : 'unfrozen'} the game. ${
...
module.exports.handleModPeekVotes = (socket, passport, game, modUserName) => {
  ...
  let output =
      '<table class="fullTable"><tr><th>Seat</th><th>Role</th><th>Vote</th></tr>';
  ...
  if (!game.private.votesPeeked) {
    ...
    ModAction
        .findOne({userActedOn : game.general.uid, actionTaken : 'Peek Votes'})
        .then(action => {
          if (action.modNotes) {
            if (action.modNotes.indexOf(passport.user) === -1) {
              action.modNotes += passport.user + '\n';
    ...
    playersToCheckVotes.map(player => {
      ...
      if (player && player.role && player.role.cardName) {
        if (player.role.cardName === 'hitler') {
          output += player.role.cardName.substring(0, 1).toUpperCase() +
                    player.role.cardName.substring(1);
        } else {
          output += player.role.cardName.substring(0, 1).toUpperCase() +
                    player.role.cardName.substring(1);
        }
      ...
      output += '</td><td>' +
                (player.isDead ? 'Dead'
                 : player.voteStatus && player.voteStatus.hasVoted
                     ? (player.voteStatus.didVoteYes ? 'Ja' : 'Nein')
                     : 'Not' +
                           ' Voted') +
                '</td>';
      output += '</tr>';
...
module.exports.handleModPeekRemakes = (socket, passport, game, modUserName) => {
  ...
  if (gameToPeek && gameToPeek.private && gameToPeek.private.seatedPlayers) {
    for (const player of gameToPeek.private.seatedPlayers) {
      if (modUserName === player.userName) {
        socket.emit('sendAlert',
                    'You cannot get votes to remake whilst playing.');
        return;
      }
    }
  }
  ...
  if (!game.private.remakeVotesPeeked) {
    ...
    ModAction
        .findOne({userActedOn : game.general.uid, actionTaken : 'Get Remakes'})
        .then(action => {
          if (action.modNotes) {
            if (action.modNotes.indexOf(passport.user) === -1) {
              action.modNotes += passport.user + '\n';
  ...
  if (gameToPeek && gameToPeek.private && gameToPeek.private.seatedPlayers) {
    ...
    playersToCheckVotes.map(player => {
      ...
      const playerRemakeData =
          game.remakeData.find(d => d.userName === player.userName);
      output += '<td>' +
                (playerRemakeData.remakeTime
                     ? moment
                           .duration(new Date() -
                                     new Date(playerRemakeData.remakeTime))
                           .humanize()
                     : '-') +
                '</td>';
 ...
 * @param {object} passport - socket authentication.
 * @param {object} data - from socket emit.
 * @param {boolean} skipCheck - true if there was an account lookup to find the
 *     IP
 * @param {array} modUserNames - list of usernames that are mods
 * @param {array} superModUserNames - list of usernames that are editors and
 *     admins
 */
module.exports.handleModerationAction = (socket, passport, data, skipCheck,
                                         modUserNames, superModUserNames) => {
  if (data.userName) {
  ...
  if (!skipCheck && !data.isReportResolveChange) {
    if (!data.ip || data.ip === '') {
      if (data.userName && data.userName !== '') {
        if (data.userName.startsWith('-')) {
          ...
          Account.findOne({username : data.userName}, (err, account) => {
            ...
            module.exports.handleModerationAction(
                socket, passport, data, true, modUserNames, superModUserNames);
          });
          return;
        }
      }
      ...
      if (data.ip.startsWith('-')) {
        ...
        data.ip = '';
        module.exports.handleModerationAction(
            socket, passport, data, false, modUserNames,
            superModUserNames); // Note: Check is not skipped here, we want to
                                // still check the username.
        return;
      }
    }
  }
  ...
  if ((!data.ip || data.ip === '') &&
      (data.action === 'timeOut' || data.action === 'ipban' ||
       data.action === 'getIP' || data.action === 'clearTimeoutIP' ||
  ...
  const isSuperMod = superModUserNames.includes(passport.user) ||
                     newStaff.editorUserNames.includes(passport.user);
  ...
  if (modUserNames.includes(passport.user) ||
    ...
    if (data.isReportResolveChange) {
      PlayerReport.findOne({_id : data._id})
          .then(report => {
            if (report) {
              ...
              report.save(() => { sendUserReports(socket); });
            }
          })
          .catch(err => { console.log(err, 'err in finding player report'); });
      ...
      if (data.comment && (data.comment.split('.').length > 1 ||
                           data.comment.split(':').length > 1)) {
        queryObj = {ip : new RegExp(`^${obfIP(data.comment.substring(1))}`)};
      } else {
        queryObj = {userActedOn : data.comment};
      }
      const userNames = userList.map(user => user.userName);

      Account
          .find({username : userNames, 'gameSettings.isPrivate' : {$ne : true}})
          .then(users => { getModInfo(users, socket, queryObj); })
          .catch(err => { console.log(err, 'err in sending mod info'); });
    } else {
      const modaction = new ModAction({
        date : new Date(),
        modUserName : passport.user,
        ...
        ip : data.ip,
        actionTaken : typeof data.action === 'string' ? data.action
                                                      : data.action.type
      });
      ...
      const logOutUser = username => {
        const bannedUserlistIndex =
            userList.findIndex(user => user.userName === username);
        ...
        Account.findOne({username})
            .then(account => {
              if (account) {
                ...
                account.save(() => {
                  const bannedAccountGeneralChats = generalChats.list.filter(
                      chat => chat.userName === username);

                  bannedAccountGeneralChats.reverse().forEach(chat => {
                    generalChats.list.splice(generalChats.list.indexOf(chat),
                                             1);
                  });
                  logOutUser(username);
                  io.sockets.emit('generalChats', generalChats);
            ...
            .catch(err => { console.log(err, 'ban user err'); });
      };
      ...
      case 'lagMeter':
        ...
        if (lagTest.length === 5) {
          socket.emit(
              'lagTestResults',
              (lagTest.reduce((acc, curr) => acc + curr, 0) / 5).toFixed(2));
          lagTest = [];
        }
        return;
      case 'clearTimeout':
        Account.findOne({username : data.userName})
            .then(account => {
              if (account) {
                ...
                socket.emit('sendAlert',
                            `No account found with a matching username: ${
                                data.userName}`);
              }
            })
            .catch(err => { console.log(err, 'clearTimeout user err'); });
        break;
      case 'warn':
        const warning = {
          time : new Date(),
            ...
            if (user.warnings && user.warnings.length > 0) {
              socket.emit('sendAlert',
                          `Warning with the message: "${
                              user.warnings.pop().text}" deleted.`);
            } else {
      ...
      case 'modEndGame':
        ...
        if (gameToEnd && gameToEnd.private && gameToEnd.private.seatedPlayers) {
          for (player of gameToEnd.private.seatedPlayers) {
            if (data.modName === player.userName) {
              socket.emit('sendAlert',
                          'You cannot end a game whilst playing in it.');
              return;
            }
          }
        }
        ...
        if (gameToEnd && gameToEnd.private && gameToEnd.private.seatedPlayers) {
          gameToEnd.chats.push({
            ...
            chat :
                'This game has been ended by a moderator, game deletes in 5 seconds.',
            isBroadcast : true,
            timestamp : new Date()
          });
          completeGame(gameToEnd, data.winningTeamName);
          setTimeout(() => {
            gameToEnd.publicPlayersState.forEach(player =>
                                                     (player.leftGame = true));
        ...
        Account.findOne({username : data.userName}).then(account => {
          if (account) {
            ...
            account.verification.email =
                account.username + '@verified.secrethitler.io';
            account.save();
          } else
            socket.emit(
                'sendAlert',
                `No account found with a matching username: ${data.userName}`);
        });
        break;
      case 'makeBypass':
        const key = createNewBypass();
        if (modaction.modNotes.length)
          modaction.modNotes += '\n';
      ...
      case 'getIP':
        if (isSuperMod) {
          ...
          socket.emit('sendAlert',
                      'Only editors and admins can request a raw IP.');
          return;
        }
        break;
      case 'rainbowUser':
        if (isSuperMod) {
          Account.findOne({username : data.userName})
              ...
              .catch(err => { console.log(err, 'rainbow user error'); });
        } else {
          socket.emit('sendAlert',
                      'Only editors and admins can rainbow a user.');
          return;
        }
        break;
      case 'deleteUser':
        ...
        if (isSuperMod) {
          ...
          Account.findOne({username : data.comment}).then(account => {
            Profile.findOne({_id : data.comment}).then(profile => {
              if (profile) {
                socket.emit('sendAlert',
                            `Profile of ${data.comment} already exists`);
                fail = true;
      ...
      case 'logoutUser':
        ...
        break;
      case 'setSticky':
        generalChats.sticky = data.comment.trim().length
                                  ? `(${passport.user}) ${data.comment.trim()}`
                                  : '';
        io.sockets.emit('generalChats', generalChats);
        break;
      case 'broadcast':
          ...
          ipban.save(() => {
            Account.find({lastConnectedIP : data.ip}, function(err, users) {
              if (users && users.length > 0) {
                users.forEach(user => { banAccount(user.username); });
              }
            });
          });
        } else {
          socket.emit('sendAlert',
                      'Only editors and admins can perform IP bans.');
          return;
        }
        break;
      case 'fragbanSmall':
        if (isSuperMod) {
          ...
          socket.emit('sendAlert',
                      'Only editors and admins can perform fragment IP bans.');
          return;
        }
        break;
      case 'fragbanLarge':
          ...
          Account.findOne({username : data.userName})
              .then(account => {
                if (account) {
                  ...
                  account.save(() => { logOutUser(data.userName); });
                } else {
                  socket.emit('sendAlert',
                              `No account found with a matching username: ${
                                  data.userName}`);
              ...
              .catch(err => { console.log(err, 'timeout user err'); });
        });
        break;
      case 'timeOut2':
        Account.findOne({username : data.userName})
            ...
            .catch(err => { console.log(err, 'timeout2 user err'); });
        break;
      case 'timeOut3':
        ...
        timeout3.save(() => {
          Account.findOne({username : data.userName})
              ...
              .catch(err => { console.log(err, 'timeout3 user err'); });
        });
        break;
      case 'timeOut4':
        Account.findOne({username : data.userName})
            ...
            .catch(err => { console.log(err, 'timeout4 user err'); });
        break;
      case 'togglePrivate':
        Account.findOne({username : data.userName})
            .then(account => {
              if (account) {
                ...
                account.gameSettings.privateToggleTime =
                    !isPrivate ? new Date('2099-01-01 00:00:00.000')
                               : Date.now();
                account.save(() => { logOutUser(data.userName); });
              } else {
                socket.emit('sendAlert',
            ...
            .catch(err => { console.log(err, 'private convert user err'); });
        break;
      ...
      case 'clearGenchat':
        if (data.userName && data.userName.length > 0) {
          generalChats.list =
              generalChats.list.filter(chat => chat.userName !== data.userName);
      ...
      case 'deleteProfile':
        if (isSuperMod) {
          ...
          Profile.findOne({_id : data.userName})
              .remove(() => { logOutUser(data.userName); })
              .catch(err => { console.log(err); });
        } else {
          socket.emit('sendAlert',
                      'Only editors and admins can delete profiles.');
          return;
        }
        break;
      case 'ipbanlarge':
        ...
        if (isSuperMod) {
          ...
          socket.emit('sendAlert',
                      'Only editors and admins can perform large IP bans.');
          return;
        }
        break;
      case 'deleteCardback':
            ...
            .then(account => {
              if (account) {
                ...
                Object.keys(games).forEach(uid => {
                  ...
                  const foundUser = game.publicPlayersState.find(
          ...
          Account.findOne({username : data.userName})
              .then(account => {
                if (account) {
                  ...
                  account.save(() => {
                    const idx =
                        newStaff.contributorUserNames.indexOf(account.username);
                    if (idx != -1)
                      newStaff.contributorUserNames.splice(idx, 1);
          ...
          Account.findOne({username : data.userName})
              .then(account => {
                if (account) {
                  ...
                  account.save(() => { logOutUser(account.username); });
                } else {
                  socket.emit('sendAlert',
                              `No account found with a matching username: ${
                                  data.userName}`);
      ...
      case 'regatherAEMList':
        if (!isSuperMod) {
          socket.emit(
              'sendAlert',
              'Only editors and admins can refresh the AEM usernames list.');
          return;
        }
        break;
      case 'resetServer':
        if (isSuperMod) {
          ...
          const crashReport = JSON.stringify({
            content : `${
                process.env
                    .DISCORDADMINPING} the site was just reset manually by an admin or editor.`
          });
          ...
          setTimeout(() => { crashServer(); }, 1000);
        } else {
          socket.emit('sendAlert',
                      'Only editors and admins can restart the server.');
          return;
        }
        break;
      default:
        if (data.userName.substr(0, 7) === 'DELGAME') {
          ...
          if (game) {
            saveAndDeleteGame(game.general.uid);
            game.publicPlayersState.forEach(
                player =>
                    (player.leftGame = true)); // Causes timed games to stop.
            sendGameList();
          }
          ...
          if (game) {
            if (modaction.modNotes.length > 0) {
              modaction.modNotes +=
                  ` - Name: "${game.general.name}" - Creator: "${
                      game.private.gameCreatorName}"`;
            } else {
              modaction.modNotes = `Name: "${game.general.name}" - Creator: "${
                  game.private.gameCreatorName}"`;
            }
          ...
          const setType = /setRWins/.test(data.action.type) ? 'rainbowWins'
                          : /setRLosses/.test(data.action.type)
                              ? 'rainbowLosses'
                          : /setWins/.test(data.action.type) ? 'wins'
                                                             : 'losses';
          const number = setType === 'wins'     ? data.action.type.substr(7)
                         : setType === 'losses' ? data.action.type.substr(9)
                         : setType === 'rainbowWins'
          ...
          if (!isNaN(parseInt(number, 10)) || isPlusOrMinus) {
            Account.findOne({username : data.userName})
                .then(account => {
                  if (account) {
                    account[setType] =
                        isPlusOrMinus
                            ? number.charAt(0) === '+'
                                  ? account[setType] + parseInt(number.substr(
                                                           1, number.length))
                                  : account[setType] - parseInt(number.substr(
                                                           1, number.length))
                            : parseInt(number);
                    ...
                    if (!data.action.isNonSeason) {
                      account[`${setType}Season${currentSeasonNumber}`] =
                          isPlusOrMinus
                              ? account[`${setType}Season${
                                    currentSeasonNumber}`]
                                    ? number.charAt(0) === '+'
                                          ? account[`${setType}Season${
                                                currentSeasonNumber}`] +
                                                parseInt(number.substr(
                                                    1, number.length))
                                          : account[`${setType}Season${
                                                currentSeasonNumber}`] -
                                                parseInt(number.substr(
                ...
                .catch(err => { console.log(err, 'set wins/losses error'); });
          }
        }
      }
      ...
      const modAction = JSON.stringify({
        content : `Date: *${new Date()}*\nStaff member: **${
            ...
            niceAction[modaction.actionTaken] ||
...
module.exports.handlePlayerReport = (passport, data, callback) => {
  ...
  if (!/^(afk\/leaving game|abusive chat|cheating|gamethrowing|stalling|botting|other)$/
           .exec(reason)) {
  ...
  const httpEscapedComment =
      data.comment.replace(/( |^)(https?:\/\/\S+)( |$)/gm, '$1<$2>$3');
  const game = games[data.uid];
  if (!game && data.uid)
    return;

  const gameType = data.uid ? (game.general.isTourny     ? 'tournament'
                               : game.general.casualGame ? 'casual'
                                                         : 'standard')
                            : 'homepage';
  ...
  const blindModeAnonymizedPlayer =
      data.uid && game.general.blindMode
  ...
  const body = JSON.stringify({
    content : `${
        data.uid ? `Game UID: <https://secrethitler.io/game/#/table/${
                       data.uid}> (${playerReport.gameType})`
                 : 'Report from homepage'}\nReported player: ${
        blindModeAnonymizedPlayer}\nReason: ${playerReport.reason}\nComment: ${
        httpEscapedComment}`,
  ...
  if (game) {
    if (!game.private.reportCounts)
      game.private.reportCounts = {};
    if (!game.private.reportCounts[passport.user])
      game.private.reportCounts[passport.user] = 0;
  ...
  try {
    ...
    console.log(
        error,
        'Caught exception in player request https request to discord server');
    reportError = true;
  }
  ...
  Account.find({staffRole : {$exists : true, $ne : 'veteran'}})
      .then(accounts => {
        accounts.forEach(account => {
          const onlineSocketId =
              Object.keys(io.sockets.sockets)
                  .find(socketId => io.sockets.sockets[socketId]
                                        .handshake.session.passport &&
                                    io.sockets.sockets[socketId]
                                            .handshake.session.passport.user ===
                                        account.username);
...
module.exports.checkUserStatus = (socket, callback) => {
  ...
  if (passport && Object.keys(passport).length) {
    ...
    const game = games[Object.keys(games).find(
        gameName => games[gameName].publicPlayersState.find(
            player => player.userName === user && !player.leftGame))];
    ...
    const reconnectingUser =
        game ? game.publicPlayersState.find(player => player.userName === user)
             : undefined;
        ...
        if (account) {
          if (account.isBanned ||
            ...
            testIP(account.lastConnectedIP, banType => {
              if (banType && banType != 'new' && banType != 'fragbanSmall' &&
...
module.exports.handleFlappyEvent = (data, game) => {
  ...
  const roomSockets =
      Object.keys(io.sockets.adapter.rooms[game.general.uid].sockets)
  ...
  if (data.type === 'startFlappy') {
    ...
    game.flappyState.pylonGenerator = setInterval(() => {
      const offset =
          Math.floor(Math.random() * 50 * game.flappyState.pylonOffset);
      const newData = {type : 'newPylon', pylonType : 'normal', offset};
  ...
  if (data.type === 'passedPylon') {
    ...
    game.general.status = `FLAPPY HITLER: ${game.flappyState.liberalScore} - ${
        game.flappyState.fascistScore} (${game.flappyState.passedPylonCount})`;

    io.sockets.in(game.general.uid).emit('gameUpdate', game);
  }
};

module.exports.handleOpenChat = (socket, data, modUserNames, editorUserNames,
                                 adminUserNames) => {
  const passport = socket.handshake.session.passport;
  if (data.aemMember !== passport.user)
    return;

  const aemMember = userList.find(x => x.userName === data.aemMember);
  if (aemMember && aemMember.staffIncognito) {
    socket.emit('sendAlert',
                'You cannot start or join a chat while Incognito.');
    return;
  }

  const dmReceiver = userList.find(x => x.userName === data.userName);
  const modInDM = Object.keys(modDMs).find(
      x => modDMs[x].subscribedPlayers.indexOf(data.aemMember) !== -1);
  const modInGame =
      Object.keys(games).find(x => games[x].gameState.isTracksFlipped &&
                                   games[x].publicPlayersState.find(
                                       y => y.userName === data.aemMember));
  ...
  if (modInDM) {
    // if the mod is already DMing someone, we should send them the DM they were
    // in instead of opening a new one
    socket.emit(
        'preOpenModDMs'); // this is necessary in order to allow the socket on
                          // the client to prepare for the openModDMs event
    socket.emit('openModDMs',
                handleAEMMessages(modDMs[dmReceiver], passport.user,
                                  modUserNames, editorUserNames,
                                  adminUserNames));
    return; // something fucky happened and they got disconnected from the chat
  }

  if (modDMs[dmReceiver.userName]) {
    // if there is an open DM but the mod is not the one who created it, they
    ...
    dm.aemOnlyMessages.push({
      ...
      staffRole : getStaffRole(passport.user, modUserNames, editorUserNames,
                               adminUserNames),
      type : 'join'
    });
    ...
    socket.emit('openModDMs',
                handleAEMMessages(modDMs[dmReceiver], data.aemMember,
                                  modUserNames, editorUserNames,
                                  adminUserNames));
    return sendInProgressModDMUpdate(dm, modUserNames, editorUserNames,
                                     adminUserNames);
  }

  const dmReceiverSocketID =
      Object.keys(io.sockets.sockets)
          .find(socketId =>
                    io.sockets.sockets[socketId].handshake.session.passport &&
                    io.sockets.sockets[socketId]
  ...
  if (dmReceiver == null || dmReceiverSocketID == null ||
      dmReceiverSocket == null) {
  ...
  dmReceiverSocket.emit('openModDMs',
                        handleAEMMessages(dmInitializeData, data.userName,
                                          modUserNames, editorUserNames,
                                          adminUserNames));
  socket.emit('preOpenModDMs');
  socket.emit('openModDMs',
              handleAEMMessages(dmInitializeData, data.aemMember, modUserNames,
                                editorUserNames, adminUserNames));
...
module.exports.handleCloseChat = (socket, data, modUserNames, editorUserNames,
                                  adminUserNames) => {
  // save, notify, etc
  const passport = socket.handshake.session.passport;

  const dmID = Object.keys(modDMs).find(
      x => modDMs[x].subscribedPlayers.indexOf(passport.user) !== -1);
  if (dmID) {
    const dm = modDMs[dmID];

    if (
        passport.user === dm.aemMember ||
        (getStaffRole(passport.user, modUserNames, editorUserNames,
                      adminUserNames) &&
         getStaffRole(passport.user, modUserNames, editorUserNames,
                      adminUserNames) !==
             'moderator') // only the mod who created the DM or any editor can
                          // close the DM
    ) {
      ...
      const dmCloseMessage =
          `__**Mod DM Closed**__\n__AEM Member__: ${dm.aemMember}\n__User__: ${
              dm.username}\n__Start Date__: ${dm.startDate}\n__End Date__: ${
              dm.endDate}\n__Chat Log__: https://secrethitler.io/modThread?id=${
              dm._id}`;
      const discordThreadNotifyBody = JSON.stringify({
...
module.exports.handleUnsubscribeChat =
    (socket, data, modUserNames, editorUserNames, adminUserNames) => {
      const passport = socket.handshake.session.passport;
      ...
      if (dm) {
        dm.aemOnlyMessages.push({
          // add leave message (this is only called for mods leaving and not the
          // DM closing)
          date : new Date(),
          chat : 'has left.',
          userName : passport.user,
        ...
        socket.emit('postCloseModDMs'); // this is necessary to allow the
                                        // force-mounted right sidebar to be
                                        // properly disposed in the right order
        sendInProgressModDMUpdate(dm, modUserNames, editorUserNames,
                                  adminUserNames);
      }
    };

module.exports.handleAddNewModDMChat =
    (socket, passport, data, modUserNames, editorUserNames, adminUserNames) => {
      const receivingPlayer = Object.keys(modDMs).find(
          x => modDMs[x].username === passport.user ||
               modDMs[x].aemMember === socket.handshake.session.passport.user);
      if (receivingPlayer) {
