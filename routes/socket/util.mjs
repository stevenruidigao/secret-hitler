...
export const getRoomSockets = game => {
	// TODO: remove
 ...
 */
export const debugSendGame = (game, message = '') => {
 ...
 */
export const testGameObject = game => {
...
export const combineCommandChats = (game, user, commandChats) => (commandChats[user] ? game.chats.concat(commandChats[user]) : game.chats);

/**
 * @param {object} game - game to act on.
 * @param {boolean} noChats - remove chats for client to handle.
 */
export const sendInProgressGameUpdate = (game, noChats = false) => {
	if (!game || !io.sockets.adapter.rooms.get(game.general.uid)) {
		return;
	}
...
export const sendInProgressModChatUpdate = (game, chat, specificUser) => {
	if (!game || !io.sockets.adapter.rooms.get(game.general.uid)) {
		return;
	}
...
export const sendPlayerChatUpdate = (game, chat) => {
	if (!game || !io.sockets.adapter.rooms.get(game.general.uid)) {
		return;
	}
...
export const sendCommandChatsUpdate = game => {
	if (!game || !io.sockets.adapter.rooms.get(game.general.uid)) {
		return;
	}
...
export const getStaffRole = (user, modUserNames, editorUserNames, adminUserNames) => {
...
export const handleAEMMessages = (dm, user, modUserNames, editorUserNames, adminUserNames) => {
...
export const sendInProgressModDMUpdate = (dm, modUserNames, editorUserNames, adminUserNames) => {
...
export const rateEloGame = (game, accounts, winningPlayerNames) => {
...
export const destroySession = username => {
...
export const handleDefaultIPv6Range = ip => {
