...
import blacklistedWords from '../iso/blacklistedWords.mts';
import bannedEmails from '../utils/disposableEmails.mts';

import { verifyRoutes, setVerify } from './verification.mts';
import prodCacheBustToken from './prodCacheBustToken.mts';

import { expandAndSimplify, obfIP, doesIPMatchCIDR } from './socket/ip-obf.mts';
...
export const accounts = (torIpsParam: any) => {
	...
	app.get('/getipintel-status', (req, res) => {
		res.send(`Current GetIPIntel counter is at ${getIPIntelCounter.count} - resets in ${(getIPIntelCounter.reset.valueOf() - new Date().valueOf()) / 1000 / 60} minutes`);
	});
